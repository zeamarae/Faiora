        ...
        // LABEL: FaioraErrorBoundary â€” Catches React rendering errors and displays a recovery UI
                ...
                if (this.state.hasError) {
                    return (
                        <div id="faiora_error_crash_screen" className="min-h-[100dvh] flex flex-col items-center justify-center p-8 text-cream-light text-center gap-4">
                            {/* crash_screen_error_msg â€” Summary: Displays the error message from the crash */}
        ...
        // Capacitor / in-app WebView: empty or bare "#" hash yields no Route match â†’ blank UI after login.
        ...
        // LABEL: ALARM-UTILITIES â€” Critical date calculation helpers for scheduling & UI readouts
        // =========================================================================================
        ...
        // --- GLOBAL UTILITY ---
        // LABEL: formatTime â€” Converts 24h time string (HH:mm) to 12h AM/PM format
        // FIX 2026-04-20: Moved to global scope to resolve "formatTime is not defined" error in Alarms/Clock page
        ...
        const groupQuickTasksBySchedule = (quickTasks = []) => {
        ...
        const buildQuickTaskStats = (quickTasks = []) => {
        ...
        // ==========================================================================
        // LABEL: FIREBASE & AUTH â€” Initialization and Google Sign-In
        // ==========================================================================
        ...
        // Move settings to absolute earliest point to ensure they are applied BEFORE persistence.
        if (isAndroidNative()) {
            db.settings({
            ...
            console.log("âœ¨ [FIREBASE] Final Stand: High-Compatibility Long-Polling enabled.");
        }

        // Enable offline persistence with multi-tab support â€” [FIX 2026-04-19] Disable synchronizeTabs on Android APK
        // to prevent potential database locks in the single-view Capacitor environment.
        db.enablePersistence({ synchronizeTabs: !isAndroidNative() }).then(() => {
            console.log("âœ¨ [FIREBASE] Persistence enabled (Native: " + isAndroidNative() + ")");
        }).catch(err => {
            if (err.code === 'failed-precondition') {
                console.warn("âš ï¸ [FIREBASE] Multi-tab persistence already active in another tab/process. Sync should be consistent.");
        ...
        const signInWithGoogle = async () => {
            if (isAndroidNative()) {
                ...
                if (result?.user) {
                    ...
                    // FIX 2026-04-22: Added specific event detail for easier tracking
        ...
        // ==========================================================================
        // LABEL: NOTIFICATION ENGINE â€” Push & Local Notifications
        // ==========================================================================
        const FaioraNotifications = (() => {
            ...
            // ------------------------------------------------------------------
            // LABEL: NOTIF-SW â€” Register Service Worker (Deep Delay to prevent extension race)
            // ------------------------------------------------------------------
                ...
                window.addEventListener('load', () => {
                    ...
                    setTimeout(() => {
                        navigator.serviceWorker.register('sw.js').then(reg => {
                            ...
                            // Suppress common extension-related registration warnings
            ...
            // ------------------------------------------------------------------
            // LABEL: NOTIF-FCM â€” VAPID Key for FCM Web Push
            // ------------------------------------------------------------------
            // HOW TO GET THIS KEY:
            //   1. Go to Firebase Console â†’ Project Settings â†’ Cloud Messaging
            //   2. Under "Web Push certificates" â†’ Click "Generate key pair"
            ...
            const ensureNativeChannels = async () => {
                ...
                try {
                    // Register action types first for stop/snooze functionality
            ...
            // ------------------------------------------------------------------
            // LABEL: NOTIF-PERMISSION â€” Request notification permission + FCM token
            // ------------------------------------------------------------------
            ...
            // ------------------------------------------------------------------
            // LABEL: NOTIF-FCM-TOKEN â€” Get FCM token and store in Firestore
            // ------------------------------------------------------------------
            // This token is what Cloud Functions use to send push to YOUR device.
            // Stored in Firestore: fcmTokens/{userId} â†’ { tokens: [...] }
            const registerFCMToken = async () => {
                try {
                    if (VAPID_KEY === 'YOUR_VAPID_KEY_HERE') {
                        console.warn('âš ï¸ FCM VAPID key not set! Background notifications won\'t work.');
                        return;
                    }

                    const messaging = firebase.messaging();
                    
                    // Ensure our sw.js is ready before getting token
            ...
            // ------------------------------------------------------------------
            // LABEL: NOTIF-LOCAL-SFX â€” Play notification sound effect
            // ------------------------------------------------------------------
            const playNotifSFX = () => {
                try {
            ...
            const playCheckSFX = () => {
                try {
                    ...
                    playTone(1046.50, 0, 0.1, 0.08); // C6
                    playTone(1318.51, 0.05, 0.15, 0.06); // E6
                } catch(e) { /* AudioContext not available */ }
            };

            // ------------------------------------------------------------------
            // LABEL: NOTIF-UTIL â€” Format time for notification title
            // ------------------------------------------------------------------
            ...
            // ------------------------------------------------------------------
            // LABEL: NOTIF-LOCAL-SEND â€” Show a local notification via Service Worker
            // ------------------------------------------------------------------
            ...
            // ------------------------------------------------------------------
            // LABEL: NOTIF-LOCAL-SCHEDULE â€” Schedule local notifications for a task
            // ------------------------------------------------------------------
            // Schedules up to 3 setTimeout timers:
            //   1. At exact due time
            //   2. 2 hours before
            //   3. 1 day before (at 9 AM)
            ...
            const scheduleForTask = async (task) => {
                ...
                // Timer 2: 1 hour before (Updated 2026-04-15)
                ...
                // Timer 3: 24 hours before (Updated 2026-04-15)
            ...
            // ------------------------------------------------------------------
            // LABEL: NOTIF-LOCAL-CANCEL â€” Cancel scheduled notifications for a task
            // ------------------------------------------------------------------
            const cancelForTask = (taskId) => {
                ...
                if (swRegistration) {
                    ['faiora-due', 'faiora-1h', 'faiora-24h'].forEach(prefix => {
                        swRegistration.getNotifications({ tag: `${prefix}-${taskId}` })
                            .then(notifs => {
            ...
            const checkCloudHealth = async () => {
                try {
                    ...
                    // Use the actual collection the app is currently using for sync
                    const activeColl = localStorage.getItem('faiora_active_collection') || 'tasks';
                    const status = { connected: false, hasToken: false };
                    
                    // 1. Connection check - Test write/read on the actual user document
                    ...
                    // If we get a permission error on write, but doc read works, maybe it's partially connected
        ...
        // ==========================================================================
        // LABEL: SHARED COMPONENTS â€” Components used across multiple pages
        // ==========================================================================
        // --- Shared Components ---
        ...
        // ------------------------------------------------------------------
        // LABEL: NAVIGATION â€” Desktop Sidebar & Mobile Bottom Nav
        // ------------------------------------------------------------------
        const ResponsiveNav = () => {
        ...
        // LABEL: PullToRefresh â€” Implements mobile "pull down to refresh" gestures
        // FIX 2026-04-20: Added labels and IDs for easier tracking
        ...
        const Layout = ({ children, onOpenCreator, onFabClick, onRefresh, noPadding = false, showFab = true, pomodoroTime, isPomodoroActive }) => {
            ...
            return (
                <div id="faiora_app_layout_shell" className="flex flex-col md:flex-row h-[100dvh] overflow-hidden bg-transparent">
                    {/* Global Pomodoro Progress Bar [FIX 2026-04-16] */}
                    {isPomodoroActive && (
                        <div id="global_pomodoro_sync_bar" className="fixed top-0 left-0 right-0 h-[3px] bg-white/5 z-[1000] pointer-events-none">
                            ...
                            ></div>
                            <div className="absolute top-0 right-0 h-full w-20 bg-gradient-to-l from-primary/30 to-transparent animate-pulse"></div>
                        </div>
                    )}
        ...
        const UserMenu = ({ user }) => {
            ...
            const [imgError, setImgError] = useState(false); // [FIX 2026-04-19] Track profile image load errors for APK fallback
            ...
            const avatarSources = useMemo(() => {
                const providerPhoto = Array.isArray(user?.providerData)
                    ? user.providerData.find(entry => entry?.photoURL)?.photoURL
                    : '';
                const rawSources = [
                    user?.photoURL,
                    providerPhoto,
            ...
            const handleRequestNotif = async () => {
             ...
             const toggleMenu = () => {
                 ...
                 if (next) {
                     ...
                     // Refresh notification status whenever menu opens
            ...
            const handleLogout = () => {
                ...
                localStorage.removeItem('faiora_uid_override'); // [FIX 2026-04-20] Clear debug overrides on logout
                localStorage.removeItem('faiora_active_collection'); // [FIX 2026-04-20] Force fresh discovery on next login
            };
           ...
           const handleLogin = () => {
            ...
            return (
                <div className="relative" ref={menuRef}>
                     <button 
                        onClick={toggleMenu}
                        className="w-10 h-10 md:w-12 md:h-12 rounded-full overflow-hidden border border-white/10 ring-4 ring-black/20 hover:ring-primary/40 transition-all duration-300 skeleton-profile-circle flex items-center justify-center bg-zinc-800"
                        id="user-profile-menu-button" // [LABEL: user-profile-menu-button] Trigger for user account and settings menu
                    >
                        {/* FIX 2026-04-19: Robust profile icon fallback for APK environment [Fix Image Load] */}
                        {activeAvatarSrc && !imgError ? (
                            ...
                            <div id="faiora_avatar_fallback" className="faiora-avatar-fallback-initials w-full h-full flex items-center justify-center bg-gradient-to-br from-primary/30 to-burnt-orange/30 text-primary font-black text-sm">
                                {(user.displayName || "U").split(' ').map(n => n[0]).join('').slice(0, 2).toUpperCase()}
                            </div>
                        )}
                        {/* FIX 2026-04-22: Removed extra brace on line 1744 that caused Babel syntax error [Fix Syntax Error] */}
                    </button>

                    {isOpen && (
                        <div id="faiora_user_menu_dropdown" className="faiora-user-menu-dropdown absolute right-0 mt-4 w-64 bg-slate-900 border border-white/20 rounded-3xl p-2 shadow-[0_25px_60px_rgba(0,0,0,0.8)] z-[250] animate-in fade-in slide-in-from-top-2 duration-200 overflow-hidden">
                            <div id="faiora_user_menu_info" className="faiora-user-menu-header p-4 border-b border-white/5 mb-2">
                                <p id="faiora_user_menu_name" className="text-white font-bold truncate">{user.displayName}</p>
                                <p id="faiora_user_menu_email" className="text-white/40 text-xs truncate">{user.email}</p>
                            </div>
                            
                            {/* Notification Control Section - HIDDEN 2026-04-15 to declutter profile menu */}
                            {false && (
                            <div className="px-3 py-2">
                                <p className="text-[10px] font-bold text-white/30 uppercase tracking-[0.2em] mb-2 px-1">Notifications</p>
                                <div className="bg-white/5 rounded-2xl p-3 border border-white/5">
                                    <div className="flex items-center justify-between mb-1">
                                        <span className="text-[11px] font-bold text-cream-light/60 uppercase tracking-widest">App Engine</span>
                                        <span className={`text-[10px] font-black uppercase px-2 py-0.5 rounded-full ${
                                            notifStatus === 'granted' ? 'bg-green-500/20 text-green-400' :
                                            notifStatus === 'denied' ? 'bg-red-500/20 text-red-400' :
                                            notifStatus === 'unsupported' ? 'bg-orange-500/20 text-orange-400' :
                                            'bg-primary/20 text-primary'
                                        }`}>
                                            {notifStatus === 'granted' ? 'Enabled' : 
                                             notifStatus === 'denied' ? 'Blocked' : 
                                             notifStatus === 'unsupported' ? 'No Support' : 'Not Set'}
                                        </span>
                                    </div>
                                    <div className="flex items-center justify-between mb-2">
                                        <span className="text-[11px] font-bold text-cream-light/60 uppercase tracking-widest">Cloud Sync</span>
                                        {cloudStatus === 'checking' ? (
                                            <div className="w-3 h-3 border-2 border-primary/40 border-t-primary rounded-full animate-spin"></div>
                                        ) : (
                                            <span className={`text-[10px] font-black uppercase px-2 py-0.5 rounded-full ${
                                                cloudStatus === 'ok' ? 'bg-blue-500/20 text-blue-400' : 'bg-red-500/20 text-red-400'
                                            }`}>
                                                {cloudStatus === 'ok' ? 'Connected' : 'Offline'}
                                            </span>
                                        )}
                                    </div>
                                    {notifStatus !== 'granted' && notifStatus !== 'unsupported' ? (
                                        <button 
                                            ...
                                            className="w-full py-2 bg-primary hover:bg-primary-dark text-white text-[10px] font-black uppercase tracking-widest rounded-xl transition-all active:scale-95 shadow-lg shadow-primary/20"
                                        >
                                            {notifStatus === 'denied' ? 'Try Enable' : 'Enable Now'}
                                        </button>
                                    ) : notifStatus === 'granted' ? (
                                        <div className="space-y-2">
                                            <p className="text-[9px] text-white/30 text-center font-medium italic">You're all set for reminders!</p>
                                            <button 
                                                onClick={() => FaioraNotifications.testNotification()}
                                                className="w-full py-2 bg-white/10 hover:bg-white/20 text-cream-light text-[9px] font-bold uppercase tracking-widest rounded-xl transition-all active:scale-95 border border-white/5"
                                            >
                                                Send Test Alert
                                            </button>
                                        </div>
                                        ...
                                        <div className="space-y-2">
                                            <p className="text-[9px] text-orange-400/80 text-center font-bold uppercase tracking-wider">PWA Required</p>
                                            <p className="text-[8px] text-white/40 text-center leading-relaxed">To get alerts on iOS, tap the <span className="text-white font-bold">Share</span> icon and select <span className="text-white font-bold">"Add to Home Screen"</span>.</p>
                                        </div>
                                    )}
                                    {notifStatus === 'denied' && (
                                        <div className="bg-red-500/10 border border-red-500/20 rounded-xl p-3 mt-2">
                                            <p className="text-[10px] text-red-400 text-center font-bold mb-1 uppercase tracking-wider">Action Required</p>
                                            <p className="text-[9px] text-white/60 text-center leading-relaxed">
                                                Your browser has blocked notifications for this site. 
                                                <br/>
                                                <span className="text-white font-bold italic">Click the "Reset permission" button</span> in your browser settings (Lock icon) to see the prompt again.
                                            </p>
                                        </div>
                                    )}
                                </div>
                            ...
                            <button id="faiora_user_menu_settings_btn" onClick={() => { setIsOpen(false); navigate('/settings'); }} className="faiora-user-menu-item flex items-center gap-3 w-full p-3 hover:bg-white/10 rounded-2xl text-white/70 hover:text-primary transition-all text-sm group">
        ...
        // LABEL: Header â€” The top navigation bar containing the title, search, and user menu
        // FIX 2026-04-20: Added labels and IDs for easier tracking
        const Header = ({
            title = "Faiora",
            ...
            return (
                <React.Fragment>
                    ...
                    `}</style>
                    {/* Mobile Sticky Header â€” FIXED 2026-04-15: Now using fixed top-0 to stay anchored at absolute top */}
                    <header className="faiora-mobile-header md:hidden fixed top-0 left-0 right-0 z-[300] bg-black/95 backdrop-blur-2xl border-b border-white/5 px-0 pt-5 pb-4 flex flex-col gap-3 animate-mobile-header">
                        <div className="flex items-center justify-between gap-3 relative z-10 w-full px-4">
                            <button
                                type="button"
                                onClick={() => {
                                    navigate('/');
                                ...
                                className="shrink-0 flex items-center justify-center text-primary transition-transform active:scale-95"
                            ...
                            {showSearch ? (
                                <div className="flex-1 relative flex items-center group">
                                    <span className="absolute left-3.5 flex items-center justify-center pointer-events-none">
                                        <span className={`${searchSpinnerActive ? 'search-loader' : ''} flex items-center justify-center`}>
                                            <span className={`material-symbols-outlined ${searchSpinnerActive ? 'text-primary' : 'text-white/40'} text-lg group-focus-within:text-primary transition-colors`}>
                                                {searchSpinnerActive ? 'refresh' : 'search'}
                                            </span>
                                        </span>
                                    </span>
                                    <input 
                                        ...
                                        className="w-full bg-white/5 border border-white/10 rounded-full py-2.5 pl-10 pr-4 text-sm text-white placeholder:text-white/20 outline-none focus:outline-none focus:ring-2 focus:ring-primary/50 focus:border-primary/50 focus:bg-white/10 transition-all font-display"
                                    />
                                </div>
                            ) : (
                                <div className="flex-1 min-w-0 px-1">
                                    <p className="text-sm font-display font-bold italic tracking-tight text-cream-light truncate">{title}</p>
                                    <p className="text-[9px] font-bold uppercase tracking-[0.28em] text-primary/60 truncate">{subtitle}</p>
                                </div>
                            )}
                    ...
                    {/* Desktop Header â€” UPDATED 2026-04-15: Non-sticky, transparent, no blur */}
                    <header id="faiora_desktop_header" className="faiora-desktop-header hidden md:flex relative top-0 md:pt-5 md:pb-3 mb-4 z-[100] bg-transparent items-center justify-between gap-8">
                        <div id="faiora_desktop_header_title_group" className="faiora-desktop-header-title flex flex-col">
                            <h1 className="text-[3.5rem] font-display font-bold tracking-tighter text-cream-light italic leading-none drop-shadow-2xl">{title}</h1>
                            <p className="text-primary/70 text-xs uppercase tracking-[0.5em] font-sans font-bold opacity-90">{subtitle}</p>
                        </div>

                        <div id="faiora_desktop_header_actions" className={`faiora-desktop-header-actions ${showSearch ? 'flex-1' : ''} flex items-center justify-end gap-10`}>
                            {showSearch && (
                                <div className="flex-1 max-w-lg relative group">
                                    <span className="absolute left-6 top-1/2 -translate-y-1/2 flex items-center justify-center pointer-events-none">
                                        <span className={`${searchSpinnerActive ? 'search-loader' : ''} flex items-center justify-center`}>
                                            <span className={`material-symbols-outlined ${searchSpinnerActive ? 'text-primary' : 'text-white/20'} text-2xl group-focus-within:text-primary transition-colors`}>
                                                {searchSpinnerActive ? 'refresh' : 'search'}
                                            </span>
                                        </span>
                                    </span>
        ...
        const TaskCreator = ({ onClose, user, editingNote, activeCollection, onUpdateNote, onDeleteNote, onSaveVersion, showToast, notes = [], onToggleLock, onOpenLockSet }) => {
            ...
            const [activeSubPopup, setActiveSubPopup] = useState(null);
             ...
             // Back button support for internal popups
             useEffect(() => {
             ...
             const handleSetActivePopup = (popup) => {
                 if (popup) {
                     ...
                     // If closing via code, and we have a popup state, we might need to pop it
                     // but handleBackButton usually handles the pop if we use browser back.
                 }
            ...
            React.useEffect(() => {
                ...
                const handleViewportChange = () => {
                    ...
                    const nextKeyboardOffset = keyboardOffset > 40 ? keyboardOffset : 0;
            ...
            // Get all unique labels from existing notes
            const allUsedLabels = React.useMemo(() => {
                const labelSet = new Set(["PRIORITY", "PERSONAL", "WORK", "URGENT", "STRATEGY"]);
                if (notes && notes.length > 0) {
                    notes.forEach(n => {
                        if (n.labels) n.labels.forEach(l => labelSet.add(l.toUpperCase()));
                    });
                }
            ...
            const icons = [
                ...
                'school', 'book', 'menu_book', 'library_books', 'bookmark', 'grade',
                'attach_money', 'account_balance', 'savings', 'credit_card', 'receipt_long',
                'flight', 'train', 'directions_car', 'directions_bike', 'directions_walk',
                'camera', 'videocam', 'headphones', 'mic', 'movie', 'tv',
                'code', 'developer_mode', 'bug_report', 'build', 'extension', 'widgets',
                'local_cafe', 'local_bar', 'local_pizza', 'bakery_dining', 'lunch_dining', 'icecream',
                'sports_basketball', 'sports_soccer', 'sports_tennis', 'pool', 'surfing', 'hiking',
                'cloud', 'wb_sunny', 'nights_stay', 'ac_unit', 'park', 'forest',
                'child_care', 'family_restroom', 'elderly', 'groups', 'person', 'face',
                'lock', 'key', 'shield', 'security', 'verified', 'admin_panel_settings',
                'notifications_active', 'alarm', 'timer', 'event', 'today', 'calendar_month',
                'chat', 'forum', 'mail', 'call', 'sms', 'send',
                'flag', 'emoji_events', 'military_tech', 'workspace_premium', 'diamond', 'token',
                'handyman', 'plumbing', 'electrical_services', 'carpenter', 'cleaning_services',
                'language', 'translate', 'travel_explore', 'map', 'pin_drop', 'place',
                'local_hospital', 'health_and_safety', 'medical_services', 'emergency',
                'church', 'mosque', 'temple_buddhist', 'synagogue',
                'edit', 'draw', 'design_services', 'color_lens', 'auto_fix_high', 'photo_filter'
            ];
            ...
            useEffect(() => {
                // Global mouse/key handlers for robustness
            ...
            const saveToFirestore = useCallback((updatedNote, forceSync = false) => {
                ...
                const newNote = {
                    ...
                    isPinned: updatedNote.isPinned !== undefined ? updatedNote.isPinned : isPinned,
                    noteTheme: updatedNote.noteTheme !== undefined ? updatedNote.noteTheme : noteTheme,
                    noteIcon: updatedNote.noteIcon !== undefined ? updatedNote.noteIcon : noteIcon,
                    labels: updatedNote.labels !== undefined ? updatedNote.labels : labels,
                    reminderDate: updatedNote.reminderDate !== undefined ? updatedNote.reminderDate : reminderDate,
                    ...
                    section: updatedNote.section !== undefined ? updatedNote.section : (editingNote && editingNote.section ? editingNote.section : ''),
                    isLocked: updatedNote.isLocked !== undefined ? updatedNote.isLocked : (editingNote?.isLocked ?? false),
                    pinHash: updatedNote.pinHash !== undefined ? updatedNote.pinHash : (editingNote?.pinHash ?? null),
                    pinHint: updatedNote.pinHint !== undefined ? updatedNote.pinHint : (editingNote?.pinHint ?? null)
                };
                ...
                if (forceSync && effectiveCollection) {
                    ...
                    if (newNote.isPublic && newNote.publicShareToken) {
                        db.collection('public_shares').doc(newNote.publicShareToken).set({
                            ...noteData,
                            ownerCollection: effectiveCollection // Store collection for guest back-sync
                        })
                        ...
                        // Delete from public_shares if toggled off (use whichever token is available)
            ...
            useEffect(() => {
                const handleGlobalKeyDown = (e) => {
                    // Only active when the app is focused on PC, but let's be safe.
                    // Search shortcut
                    if ((e.ctrlKey || e.metaKey) && e.key.toLowerCase() === 'f') {
                        e.preventDefault();
                        setIsSearchOpen(prev => !prev);
                        if (!isSearchOpen) setSearchQuery(''); // Clear on open
                        return;
                    }
            ...
            }, [history, historyIndex, isSearchOpen]); // Add dependencies to fix stale closure
            ...
            useEffect(() => {
                ...
                const unsubscribe = db.collection('public_shares').doc(publicShareToken).onSnapshot(doc => {
                    if (doc.exists) {
                        ...
                        if (data.isPinned !== undefined && data.isPinned !== isPinned) {
                            setIsPinned(data.isPinned);
                        }
                        if (data.noteTheme !== undefined && data.noteTheme !== noteTheme) {
                            setNoteTheme(data.noteTheme);
                        }
                        if (data.noteIcon !== undefined && data.noteIcon !== noteIcon) {
                            setNoteIcon(data.noteIcon);
                        }
                        if (data.labels !== undefined && JSON.stringify(data.labels) !== JSON.stringify(labels)) {
                            setLabels(data.labels);
                        }
                        if (data.reminderDate !== undefined && data.reminderDate !== reminderDate) {
            ...
            useEffect(() => {
                const container = document.querySelector('.task-creator-modal .overflow-y-auto');
                if (container) {
            ...
            const triggerClose = (shouldShowToast = false) => {
                setIsClosing(true);
                setTimeout(() => {
                    if (shouldShowToast && showToast) showToast("Changes Saved");
                    onClose();
                }, 230);
            };

            const handleClose = () => {
                if (isSubmitting) return; // Prevent double closure/save
                const html = editorRef.current ? editorRef.current.innerHTML : content;
            ...
            const saveState = useCallback(() => {
                ...
                saveToFirestore(currentState, true); // Force immediate Firestore sync for real-time updates
                setHasChanges(true);
                hasChangesRef.current = true;
            ...
            const convertActiveBlock = (type) => {
                ...
                if (type === 'h1' || type === 'h2') {
                    ...
                    if (inListItem && range.collapsed) {
                        ...
                        // BATCH CONVERSION â€” collect items from DOM BEFORE any mutation
                        ...
                        if (parentList) {
                            ...
                            if (selectedLIs.length === allLIs.length) {
                                // All items selected â€” remove entire list
                                ...
                                // Partial selection â€” replace only selected LIs, keep list
                                const frag = document.createDocumentFragment();
                                lines.forEach(l => {
                                    const wrapper = document.createElement('div');
                                    wrapper.className = 'checklist-item';
                            ...
                            // Source is plain text / divs / headings â€” use extract approach
            ...
            const handleEditorKeyDown = (e) => {
                ...
                // --- Space Key Logic (for list auto-formatting) ---
                if (e.key === ' ') {
                ...
                if (e.key === 'Enter') {
                    ...
                    if (!item) {
                        ...
                        if (currentBlock && !currentBlock.classList.contains('note-editor-area')) {
                            ...
                            // Smart Exit: If user presses Enter on an empty bullet line or an empty indented line
                ...
                if (e.key === 'Backspace' && range.collapsed) {
                    ...
                    if (currentBlock && startOffset === 0) {
                        // Check for indentation or special class
                        const hasIndentation = currentBlock.style.marginLeft && currentBlock.style.marginLeft !== '0px';
                        const isBulletItem = currentBlock.classList.contains('bullet-item');
                        ...
                        if (hasIndentation || isBulletItem || isChecklistItem) {
                            ...
                            if (isChecklistItem) {
                                ...
                                // Remove bullet class but keep content
            ...
            const applyCase = (caseType) => {
                ...
                while (walker.nextNode()) {
                    ...
                    if (caseType === 'upper') {
                        ...
                        // Better capitalize: Force Title Case (every word starts with upper, rest lower)
                ...
                // Restore selection across the modified content
            ...
            const handleImageResize = (e, targetImage, handleType) => {
                ...
                const onMove = (moveEvent) => {
                    ...
                    // Resizing logic with aspect ratio preservation
                    let deltaX = 0;
                    let deltaY = 0;
                    ...
                    // Use the larger delta to drive resizing
                    ...
                    targetImage.style.height = 'auto'; // Keep aspect ratio
            ...
            const handleImageMoveStart = (e, container) => {
                const isTouch = e.type.startsWith('touch');
                const startX = isTouch ? e.touches[0].clientX : e.clientX;
                const startY = isTouch ? e.touches[0].clientY : e.clientY;
                ...
                container.classList.add('dragging-now');
                ...
                const onMove = (moveEvent) => {
                    ...
                    if (document.caretRangeFromPoint) {
                        // Offset Y slightly to target the line center better
                    ...
                    if (range && editorRef.current.contains(range.startContainer)) {
                        // Ensure it's inside a block-level element if possible
                        let container = range.startContainer;
                        if (container.nodeType === 3) container = container.parentNode;
                        
                        // Avoid deep nesting inside complex spans if we can
                        const block = container.closest('div, p, li, h1, h2');
                        if (block && editorRef.current.contains(block)) {
                             // Correct range if we are at edge of text node
                ...
                const onEnd = () => {
                    ...
                    container.classList.remove('dragging-now');
                ghost.remove();
            ...
            useEffect(() => {
                const handleGlobalInteract = (e) => {
                    ...
                    if (target.classList.contains('resizable-image')) {
                        ...
                        // Long Press to Move
                        const timer = setTimeout(() => {
                            if (container) handleImageMoveStart(e, container);
                        }, 500);
                    ...
                    // 2. Handle clicking a resize handle (Remove crop handles)
                    if (target.classList.contains('resize-handle')) {
                        e.stopPropagation();
                        e.preventDefault();
                        const handleType = target.classList.contains('tl') ? 'tl' :
                    ...
                    // 4. Handle Action Buttons (Delete)
                    ...
                    if (action) {
                        ...
                        if (action === 'image-delete' && container) {
                            container.remove();
                            setHasChanges(true);
                            hasChangesRef.current = true;
                            saveState();
            ...
            /* notepad-modal-overlay - fullscreen overlay container for the task creator/notepad */
            /* notepad-modal-overlay - raised z-index to 999 for extreme mobile priority */
            /* FIX 2026-04-15: Raised to z-[999] (was 500) */
            return (
                <div className={"fixed inset-0 z-[999] flex items-center justify-center transition-all duration-300 " + (isFullscreen ? "p-0" : "p-0 md:p-8") + " " + (isClosing ? 'opacity-0 scale-95 pointer-events-none' : 'animate-in fade-in duration-500')}>
                    <div className={"absolute inset-0 bg-black/70 backdrop-blur-2xl transition-opacity duration-300 " + (isClosing ? 'opacity-0' : '')} onClick={handleClose}></div>
                    
                    <div ref={modalRef} style={{ transform: 'translateY(calc(-1 * var(--faiora-keyboard-offset, 0px)))' }} className={"task-creator-modal w-full max-w-full h-full overflow-hidden flex flex-col relative z-10 shadow-2xl transition-all duration-250 " + (isFullscreen ? "md:max-w-full md:h-[100dvh] md:rounded-none !p-0 !m-0" : "md:max-w-4xl md:h-[85vh] md:rounded-[2rem]") + " " + (isClosing ? 'opacity-0 scale-95 translate-y-20 md:translate-y-0 ease-in' : 'animate-in zoom-in-95 duration-500 ease-out') + ' transition-colors duration-500 ' + currentThemeClass}>
                        ...
                        >
                            ...
                            >
                                <span className={`text-[9px] font-bold text-slate-800 px-2 mb-0.5 select-none transition-opacity ${progress < 15 ? 'opacity-0' : 'opacity-100'}`}>
                                    {progress}%
                                </span>
                                <div className="absolute right-0 top-1/2 -translate-y-1/2 w-2 h-2 bg-white rounded-full shadow-lg scale-0 group-hover:scale-100 transition-transform ring-4 ring-primary/20"></div>
                            </div>
                            {progress < 15 && (
                                <span className="absolute left-2 top-1/2 -translate-y-1/2 text-[9px] font-bold text-slate-800 opacity-60 select-none">
                                    {progress}%
                                </span>
                            )}
                        </div>
                        ...
                        >
                            ...
                            <div className="flex justify-between items-start mb-4 relative z-10">
                                <div className="flex-1 flex flex-col gap-2">
                                    ...
                                    <div className="flex flex-wrap items-center gap-2 px-0 md:px-4">
                                        {labels.map(label => (
                                            <span key={label} className="text-[10px] font-bold uppercase tracking-widest bg-primary/10 text-primary px-2 py-1 rounded-md flex items-center gap-1 font-montserrat">
                                                {label}
                                                {/* FIX 2026-04-15: label-remove-btn - saves updated labels directly to Firestore instead of stale saveState */}
                                                {canEdit && (
                                                    <button onClick={() => { const newLabels = labels.filter(l => l !== label); setLabels(newLabels); setHasChanges(true); hasChangesRef.current = true; saveToFirestore({ title, content: (editorRef.current ? editorRef.current.innerHTML : content), progress, isPinned, noteTheme, noteIcon, labels: newLabels, reminderDate, section: (editingNote ? editingNote.section : '') || '' }, true); }} className="hover:text-primary-dark">
                                ...
                                <div className="flex items-center gap-0.5 flex-shrink-0">
                                    <button 
                                        onClick={() => { if(!canEdit) return; setIsPinned(!isPinned); setHasChanges(true); hasChangesRef.current = true; saveState(); }}
                                        className={"p-2 rounded-full transition-all " + (isPinned ? "text-primary bg-primary/10" : "text-slate-400 hover:text-slate-600")}
                                        disabled={!canEdit}
                                    >
                                        <span className="material-symbols-outlined text-[22px] md:text-[24px]" style={isPinned ? {fontVariationSettings: "'FILL' 1"} : {}}>push_pin</span>
                                    </button>
                                    <button 
                                        onClick={handleClose}
                            ...
                            {isSearchOpen && (
                                <div className="note-search-bar fixed top-4 right-4 md:right-12 z-[200] flex items-center gap-1.5 bg-white/95 backdrop-blur-2xl p-1.5 pr-2 rounded-xl border border-black/10 shadow-[0_8px_30px_rgb(0,0,0,0.12)] animate-in slide-in-from-top-4 zoom-in-95 duration-300 ring-1 ring-black/5">
                                    <div className="flex items-center flex-1 bg-black/[0.03] rounded-lg px-2 py-1">
                                    ...
                                    {totalMatches > 0 && (
                                        <span className="text-[10px] font-bold text-slate-400 font-montserrat flex-shrink-0 px-1">
                                            {((currentMatchIndex % totalMatches + totalMatches) % totalMatches) + 1}/{totalMatches}
                                        </span>
                                    )}
                                    <div className="flex items-center gap-0.5 border-l border-black/5 pl-1 ml-0.5">
                                        <button 
                            ...
                            <div className="flex-1">
                                <div 
                                    ...
                                    style={{ paddingBottom: `${keyboardSafeSpace}px`, scrollPaddingBottom: `${keyboardSafeSpace}px` }}
                        ...
                        <div className="task-creator-footer mt-auto flex items-center justify-between p-1 md:p-2 bg-white/50 backdrop-blur-xl border-t border-black/5 relative z-[100] gap-1 md:gap-2 overflow-visible">
                            {/* Left Side: Main Tools Group */}
                            <div className="flex items-center gap-0 md:gap-1 flex-nowrap overflow-visible">
                                {/* Nested Horizontal Formatting Dropdown */}
                                 <div className="relative flex-shrink-0">
                                    {/* notepad-formatting-trigger [FIX 2026-04-16] */}
                                    <button 
                                        id="notepad_format_btn"
                                        onClick={() => { if(!canEdit) return; setActivePopup(activePopup === 'format' ? null : 'format'); }} 
                                        ...
                                        disabled={!canEdit}
                                    >
                                        <span className="material-symbols-outlined text-[22px]">format_color_text</span>
                                    </button>
                                    
                                    {activePopup === 'format' && (
                                        <div className="format-popup-bar absolute bottom-[calc(100%+8px)] left-1 bg-white rounded-2xl pl-3 shadow-xl border border-black/5 flex flex-nowrap items-center p-2 z-[150] animate-in slide-in-from-bottom-2 duration-200">
                                                ...
                                                {/* Style Sub-menu Trigger */}
                                                 <div className="relative px-2">
                                                     ...
                                                     >
                                                        <span className="text-[14px] font-bold tracking-tight uppercase font-montserrat">H1</span>
                                                        <span className="material-symbols-outlined text-sm">arrow_drop_up</span>
                                                    </button>
                                                    {/* Style Horizontal Bar */}
                                                    {activeSubPopup === 'style' && (
                                                        <div className="absolute bottom-full left-0 mb-2 bg-white rounded-xl shadow-2xl border border-black/10 flex items-center p-1.5 px-3 z-[160] animate-in slide-in-from-bottom-2 duration-200 gap-2 w-max whitespace-nowrap">
                                                        <button onMouseDown={(e) => { e.preventDefault(); convertActiveBlock('h1'); }} className="h-9 px-2 flex items-center hover:bg-black/5 rounded-md text-base font-bold text-slate-700 font-montserrat">H1</button>
                                                        <button onMouseDown={(e) => { e.preventDefault(); convertActiveBlock('h2'); }} className="h-9 px-2 flex items-center hover:bg-black/5 rounded-md text-sm font-bold text-slate-600 font-montserrat">H2</button>
                                                        <button onMouseDown={(e) => { e.preventDefault(); convertActiveBlock('normal'); }} className="centered h-9 px-6 flex items-center text-center justify-center hover:bg-black/5 rounded-md text-xs text-slate-500 font-montserrat">Normal</button>
                                                        </div>
                                                    )}
                                                </div>

                                                <div className="fmt-divider w-px h-6 bg-black/10 mx-1"></div>
                                                ...
                                                <div className="relative">
                                                     ...
                                                     >
                                                        <span className="text-[14px] font-bold tracking-tight font-montserrat">Aa</span>
                                                        <span className="material-symbols-outlined text-sm">arrow_drop_up</span>
                                                    </button>
                                                    {/* Case Horizontal Bar */}
                                                    {activeSubPopup === 'case' && (
                                                        <div className="absolute bottom-full left-0 mb-2 bg-white rounded-xl shadow-2xl border border-black/10 flex items-center p-1.5 px-2 z-[150] animate-in slide-in-from-bottom-2 duration-200 gap-1">
                                                            <button onMouseDown={(e) => { e.preventDefault(); applyCase('cap'); }} className="h-5 px-3 flex items-center hover:bg-black/5 rounded-lg text-xs capitalize font-bold text-slate-700 font-montserrat">Aa</button>
                                                            <button onMouseDown={(e) => { e.preventDefault(); applyCase('upper'); }} className="h-5 px-3 flex items-center hover:bg-black/5 rounded-lg text-xs uppercase font-bold text-slate-700 font-montserrat">AA</button>
                                                            <button onMouseDown={(e) => { e.preventDefault(); applyCase('lower'); }} className="h-5 px-3 flex items-center hover:bg-black/5 rounded-lg text-xs lowercase font-bold text-slate-700 font-montserrat">aa</button>
                                                        </div>
                                                    )}
                                                </div>
                                                ...
                                                <div className="flex items-center">
                                                    <button onMouseDown={(e) => { e.preventDefault(); toggleFormat('**'); }} className={"w-10 h-10 flex items-center justify-center rounded-lg transition-colors " + (activeFormats.bold ? "bg-black/10 text-primary scale-110" : "hover:bg-black/5 text-slate-600")} title="Bold">
                                                        <span className="material-symbols-outlined text-[20px]" style={activeFormats.bold ? {fontVariationSettings: "'WGHT' 700"} : {}}>format_bold</span>
                                                    </button>
                                                    <button onMouseDown={(e) => { e.preventDefault(); toggleFormat('*'); }} className={"w-10 h-10 flex items-center justify-center rounded-lg transition-colors " + (activeFormats.italic ? "bg-black/10 text-primary scale-110" : "hover:bg-black/5 text-slate-600")} title="Italic">
                                                        <span className="material-symbols-outlined text-[20px]">format_italic</span>
                                                    </button>
                                                    <button onMouseDown={(e) => { e.preventDefault(); toggleFormat('__'); }} className={"w-10 h-10 flex items-center justify-center rounded-lg transition-colors " + (activeFormats.underline ? "bg-black/10 text-primary scale-110" : "hover:bg-black/5 text-slate-600")} title="Underline">
                                                ...
                                                <div className="fmt-divider w-px h-6 bg-black/5 mx-1"></div>
                                                ...
                                                <div className="relative">
                                                    ...
                                                    {/* Highlight Color Picker â€” positioned to stay on screen */}
                                                    {activeSubPopup === 'highlight' && (
                                                        <div className="absolute bottom-full right-0 mb-2 p-2 bg-white rounded-xl shadow-2xl border border-black/5 flex items-center z-[160] animate-in slide-in-from-bottom-1 duration-200" style={{ minWidth: '130px' }}>
                                                            <div className="grid grid-cols-4 gap-2">
                                                                <button 
                                                                    ...
                                                                    title="No Highlight"
                                                                >
                                                                    <span className="material-symbols-outlined text-xs text-slate-200">format_color_reset</span>
                                                                </button>
                                                ...
                                                <div className="relative">
                                                    ...
                                                    >
                                                        <span className="material-symbols-outlined text-[20px]" style={{color: '#ea580c'}}>horizontal_rule</span>
                                                        <span className="material-symbols-outlined text-sm">arrow_drop_up</span>
                                                    </button>
                                                    {/* Divider Color Picker â€” positioned to stay on screen */}
                                                    {activeSubPopup === 'divider' && (
                                                        <div className="absolute bottom-full right-0 mb-2 p-2 bg-white rounded-xl shadow-2xl border border-black/5 flex items-center gap-2 z-[160] animate-in slide-in-from-bottom-1 duration-200">
                                                            {['#ea580c', '#334155', '#64748b', '#94a3b8', '#cbd5e1'].map(color => (
                                                                <button 
                                                                    key={color}
                                                                    onMouseDown={(e) => { 
                                                                        e.preventDefault(); 
                                                                    ...
                                                                    className="divider-option w-6 h-6 rounded-full border border-black/5 hover:scale-110 transition-transform flex-shrink-0"
                                                                    style={{ backgroundColor: color }}
                                                                    title={color === '#ea580c' ? 'Dark Orange' : 'Gray Shade'}
                                                                />
                                                            ))}
                                                        </div>
                                                    )}
                                ...
                                {/* 2. Tools Dropdown (Overflow Menu) */}
                                 <div className="relative flex-shrink-0">
                                     <button onClick={() => handleSetActivePopup(activePopup === 'tools' ? null : 'tools')} className={"footer-action-btn flex items-center justify-center transition-all rounded-xl " + (activePopup === 'tools' ? "bg-black/5 text-secondary" : "text-secondary hover:bg-black/5")} title="More Tools">
                                         <span className="material-symbols-outlined text-[26px]">menu</span>
                                     </button>
                                    {activePopup === 'tools' && (
                                        <div className="absolute bottom-[calc(100%+8px)] left-0 md:left-0 md:translate-x-0 bg-white rounded-2xl shadow-xl border border-black/5 flex items-center p-1 z-[150] animate-in slide-in-from-bottom-2 duration-200">
                                                 ...
                                                 <div className="flex items-center px-1">
                                                     <button onClick={() => { convertActiveBlock('todo'); }} className="w-10 h-10 flex items-center justify-center hover:bg-black/5 rounded-lg text-slate-600" title="Checklist"><span className="material-symbols-outlined text-[20px]">checklist</span></button>
                                                     <button onClick={() => { convertActiveBlock('bullet'); }} className="w-10 h-10 flex items-center justify-center hover:bg-black/5 rounded-lg text-slate-600" title="Bullets"><span className="material-symbols-outlined text-[20px]">format_list_bulleted</span></button>
                                                     <button onClick={() => { convertActiveBlock('number'); }} className="w-10 h-10 flex items-center justify-center hover:bg-black/5 rounded-lg text-slate-600" title="Numbering"><span className="material-symbols-outlined text-[20px]">format_list_numbered</span></button>
                                                 </div>
                                                 <div className="w-px h-6 bg-black/5 mx-1"></div>
                                                 <div className="flex items-center px-1">
                                                     <button onClick={() => { fileInputRef.current.click(); }} className="w-10 h-10 flex items-center justify-center hover:bg-black/5 rounded-lg text-slate-600" title="Add Image"><span className="material-symbols-outlined text-[20px]">image</span></button>
                                                 </div>
                                             </div>
                                    )}
                                </div>
                                
                                <div className="footer-divider flex-shrink-0 hidden md:block"></div>

                                {/* Rest of Footer Icons */}
                                 <div className="relative flex-shrink-0">
                                     <button onClick={() => handleSetActivePopup(activePopup === 'reminder' ? null : 'reminder')} className={"footer-action-btn relative " + (activePopup === 'reminder' ? "bg-black/5 text-primary" : "")} title="Reminders">
                                         <span className="material-symbols-outlined text-[24px]">notifications</span>
                                        {reminderDate && <span className="absolute top-1 right-1 w-2.5 h-2.5 bg-orange-500 rounded-full border-2 border-white"></span>}
                                    </button>
                                    {activePopup === 'reminder' && (
                                        <div className="absolute bottom-full mb-4 left-0 bg-white rounded-xl shadow-2xl border border-black/5 z-[150] w-72 animate-in slide-in-from-bottom-2 duration-300 py-2">
                                            {!showCustomReminder ? (
                                                <div className="flex flex-col">
                                                    ...
                                                    <button onClick={() => { 
                                                        ...
                                                        setHasChanges(true); hasChangesRef.current = true; handleSetActivePopup(null); 
                                                    ...
                                                    <button onClick={() => { 
                                                        ...
                                                        setReminderDate(formatDateLocal(tomorrow) + 'T08:00'); setHasChanges(true); hasChangesRef.current = true; handleSetActivePopup(null); 
                                                    }} className="px-5 py-3 hover:bg-black/5 flex items-center justify-between text-sm text-slate-600 transition-colors font-montserrat">
                                                        <span>Tomorrow</span>
                                                    ...
                                                    <button onClick={() => { 
                                                        if (!reminderDate) {
                                                            // Default to next Monday 10:00 AM
                                                            const now = new Date();
                                                    ...
                                                    }} className="px-5 py-3 hover:bg-black/5 flex items-center gap-3 text-sm text-slate-700 font-semibold transition-colors font-montserrat justify-between">
                                                        <div className="flex items-center gap-3">
                                                            <span className="material-symbols-outlined text-lg">schedule</span>
                                                            <span className="font-montserrat">Custom</span>
                                                        </div>
                                                        {reminderDate && <span className="w-2.5 h-2.5 bg-orange-500 rounded-full flex-shrink-0"></span>}
                                                    </button>
                                                </div>
                                            ) : (
                                                <div className="p-4">
                                                    <div className="flex items-center gap-2 mb-4">
                                                        ...
                                                        <span className="text-sm font-bold text-slate-700 font-montserrat">Custom</span>
                                                    </div>
                                                        ...
                                                        <div className="relative border-b border-black/10 focus-within:border-primary transition-colors">
                                                            <input 
                                                                ...
                                                                onChange={(e) => { 
                                                                    ...
                                                                    const currentDate = reminderDate.split('T')[0] || formatDateLocal(new Date());
                                                    ...
                                                    <div className="flex justify-end mt-6">
                                                        <button onClick={() => { setHasChanges(true); hasChangesRef.current = true; handleSetActivePopup(null); }} className="px-6 py-2 text-sm font-bold text-slate-800 hover:bg-black/5 rounded-lg transition-all font-montserrat">
                                                            Save
                                                        </button>
                                                    </div>
                                                </div>
                                 ...
                                 <div className="relative flex-shrink-0">
                                     <button onClick={() => handleSetActivePopup(activePopup === 'palette' ? null : 'palette')} className={"footer-action-btn " + (activePopup === 'palette' ? "bg-black/5 text-black" : "")} title="Theme">
                                         <span className="material-symbols-outlined text-[24px]">palette</span>
                                     </button>
                                    {activePopup === 'palette' && (
                                        <div className="absolute bottom-full mb-4 left-0 md:left-0 md:translate-x-0 grid grid-cols-6 gap-2 p-3 w-max bg-white rounded-2xl shadow-2xl border border-black/5 z-[150] animate-in slide-in-from-bottom-2 duration-200">
                                            {themes.map(t => (
                                                <button 
                                                    key={t.id}
                                                    onClick={() => { setNoteTheme(t.id); handleSetActivePopup(null); setHasChanges(true); hasChangesRef.current = true; saveState(); }}
                                ...
                                />
                                <div className="relative flex-shrink-0">
                                    <button onClick={() => handleSetActivePopup(activePopup === 'icon' ? null : 'icon')} className={"footer-action-btn " + (activePopup === 'icon' ? "bg-black/5 text-primary" : "")} title="Choose Icon">
                                        <span className="material-symbols-outlined text-[24px]">add_reaction</span>
                                    </button>
                                    {activePopup === 'icon' && (
                                        <div className="absolute bottom-full mb-4 left-1/2 -translate-x-1/2 md:left-0 md:translate-x-0 flex flex-col p-3 bg-white rounded-2xl shadow-2xl border border-black/5 z-[120] w-72 md:w-80 animate-in slide-in-from-bottom-2 duration-200">
                                            ...
                                            <div className="max-h-56 overflow-y-auto icon-picker-scroll grid grid-cols-4 gap-2 w-full pr-1 px-1">
                                                {filteredIcons.map(icon => (
                                                    <button 
                                                        key={icon}
                                                        onClick={() => { setNoteIcon(icon); handleSetActivePopup(null); setHasChanges(true); hasChangesRef.current = true; saveState(); }}
                                            ...
                                            <button 
                                                onClick={() => { setNoteIcon(""); handleSetActivePopup(null); setHasChanges(true); hasChangesRef.current = true; saveState(); }}
                            ...
                            {/* Right Side: Undo, Redo, More */}
                             <div className="flex items-center gap-1 md:gap-2">
                                 <button onClick={() => { setIsSearchOpen(!isSearchOpen); if(isSearchOpen) setSearchQuery(''); }} className={"footer-action-btn flex-shrink-0 " + (isSearchOpen ? "text-primary bg-primary/10" : "")} title="Find (Ctrl+F)">
                                     <span className="material-symbols-outlined text-[24px]">search</span>
                                 </button>
                                 <button onClick={() => setIsFullscreen(!isFullscreen)} className={"footer-action-btn flex-shrink-0 hidden md:flex " + (isFullscreen ? "text-primary bg-primary/10" : "")} title="Toggle Fullscreen">
                                 ...
                                 <div className="relative flex-shrink-0">
                                     <button onClick={(e) => { e.stopPropagation(); handleSetActivePopup(activePopup === 'more' ? null : 'more'); }} className={"footer-action-btn " + (activePopup === 'more' ? "bg-black/5 text-primary" : "")} title="More Options">
                                         <span className="material-symbols-outlined text-[24px]">more_vert</span>
                                     </button>
                                     {activePopup === 'more' && (
                                         <div className="more-menu-dropdown z-[120] animate-in slide-in-from-bottom-2 duration-200">
                                             <button onClick={() => { handleSetActivePopup('labels'); }} className="menu-item font-montserrat">
                                                 <span className="material-symbols-outlined text-lg">label</span>
                                                 Add Label
                                             </button>
                                             <button onClick={() => { handleSetActivePopup('share'); }} className="menu-item font-montserrat">
                                             ...
                                             <button onClick={() => { handleSetActivePopup('history'); }} className="menu-item font-montserrat">
                                             ...
                                             <button 
                                                onClick={() => {
                                                    handleSetActivePopup(null);
                                                    if (isLocked) {
                                                        onToggleLock(); // This will trigger the unlock flow
                                                    } else {
                                                        onOpenLockSet(); // This will open the set lock modal
                                                    }
                                                }} 
                                                className="menu-item font-montserrat"
                                             >
                                                 <span className="material-symbols-outlined text-lg">{isLocked ? 'lock_open' : 'lock'}</span>
                                     ...
                                     {activePopup === 'labels' && (
                                         <div className="more-menu-dropdown z-[120] w-56 animate-in scale-95 duration-200">
                                             <div className="flex items-center gap-2 mb-3 p-1">
                                                 <button onClick={() => handleSetActivePopup('more')} className="p-1 hover:bg-slate-100 rounded-full">
                                             ...
                                             <div className="flex flex-col gap-1 mb-2 max-h-40 overflow-y-auto no-scrollbar">
                                                 {allUsedLabels.map(l => (
                                                     ...
                                                     }} className="menu-item !py-2 justify-between font-montserrat">
                                                         <span>{l}</span>
                                     ...
                                     {activePopup === 'share' && (
                                         <div className="more-menu-dropdown z-[120] w-80 p-0 overflow-hidden animate-in slide-in-from-bottom-2 duration-200">
                                             <div className="p-4 border-b border-slate-100 flex items-center justify-between bg-white">
                                                 <div className="flex items-center gap-2">
                                                     <button onClick={() => handleSetActivePopup('more')} className="p-1 hover:bg-slate-100 rounded-full">
                                                         <span className="material-symbols-outlined text-sm">arrow_back</span>
                                                     </button>
                                                     <span className="text-sm font-bold text-slate-800 font-montserrat tracking-tight">Share Note</span>
                                                 </div>
                                                 <button onClick={() => handleSetActivePopup(null)} className="text-slate-400 hover:text-slate-600">
                                             ...
                                             <div className="p-4 space-y-5 bg-white">
                                                 {/* Email Sharing */}
                                                 <div>
                                                     <label className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-2 block">Invite by Email</label>
                                                     ...
                                                     <div className="mt-3 flex flex-wrap gap-2">
                                                         {sharedWith.map(email => (
                                                             <span key={email} className="inline-flex items-center gap-1.5 bg-slate-100 text-slate-600 px-2 py-1 rounded-lg text-[10px] font-bold">
                                                                 {email}
                                                                 <button onClick={() => { 
                                                 ...
                                                 <div className="space-y-3">
                                                     <div className="flex items-center justify-between">
                                                         <div className="flex flex-col">
                                                             <span className="text-sm font-bold text-slate-700 font-montserrat">Public Access</span>
                                                             <span className="text-[10px] text-slate-400 font-montserrat">Anyone with the link can view</span>
                                                         </div>
                                                         <button 
                                                             onClick={() => {
                                                     ...
                                                     {isPublic && (
                                                         <>
                                                             <div className="flex items-center justify-between animate-in fade-in slide-in-from-top-1 duration-200">
                                                                 <div className="flex flex-col">
                                                                     <span className="text-sm font-bold text-slate-700 font-montserrat">Allow Public Editing</span>
                                                                     <span className="text-[10px] text-slate-400 font-montserrat">Anyone can edit this note</span>
                                                                 </div>
                                                                 <button 
                                                                     onClick={() => { 
                                                                 ...
                                                                 >
                                                                     <div className={`absolute top-1 w-3 h-3 bg-white rounded-full transition-all ${allowPublicEdit ? 'left-6' : 'left-1'}`}></div>
                                                                 </button>
                                                             </div>
                                                             <button 
                                                                 onClick={() => {
                                     ...
                                     {activePopup === 'history' && (
                                         <div className="more-menu-dropdown z-[120] w-72 p-0 overflow-hidden animate-in slide-in-from-bottom-2 duration-200">
                                             <div className="p-4 border-b border-slate-100 flex items-center justify-between bg-slate-50">
                                                 <div className="flex items-center gap-2">
                                                     <button onClick={() => handleSetActivePopup('more')} className="p-1 hover:bg-slate-200 rounded-full">
                                                         <span className="material-symbols-outlined text-sm">arrow_back</span>
                                                     </button>
                                             ...
                                             <div className="max-h-64 overflow-y-auto no-scrollbar p-1">
                                                  {Object.entries(editingNoteRef.current?.versions || {}).sort((a,b) => b[0].localeCompare(a[0])).map(([dayId, version]) => (
                                                      <div key={dayId} className="p-3 hover:bg-slate-50 rounded-xl flex items-center justify-between group">
                                                          <div className="flex flex-col">
                                                              <div className="flex items-center gap-2">
                                                                  <span className="material-symbols-outlined text-slate-400 text-sm">event</span>
                                                                  <span className="text-xs font-bold text-slate-700">{dayId}</span>
                                                              </div>
                                                              <span className="text-[10px] text-slate-400 ml-6 uppercase tracking-wider font-bold">Daily Snapshot</span>
                                                          </div>
                                                          <button 
                                                              onClick={() => {
                                             ...
                                             <div className="p-3 bg-slate-50 flex justify-end">
                                                  <button onClick={() => handleSetActivePopup(null)} className="text-xs font-bold text-slate-800 hover:text-primary">Dismiss</button>
                                             </div>
                                         </div>
                                     )}
                                 </div>
        ...
        const LoginModal = () => {
            ...
            /* FIX 2026-04-15: Raised to z-[999] (was 500) */
            return (
                <div className="faiora-auth-screen fixed inset-0 z-[999] flex items-center justify-center p-6 blur-overlay overflow-hidden">
        ...
        const ConfirmationModal = ({ title, message, onConfirm, onCancel, confirmText = "Confirm", cancelText = "Cancel", type = "danger" }) => {
            ...
            /* FIX 2026-04-15: Raised to z-[999] (was 500) */
            return (
                <div className="fixed inset-0 z-[999] flex items-center justify-center p-4 md:p-10 modal-overlay animation-fade-in">
        ...
        const SkeletonNoteCard = ({ variant = 'full', index }) => {
            const isPriority = variant === 'priority';
            const isCompact = variant === 'compact';
            
            return (
                <div className={`animate-pulse rounded-[2.2rem] border-b-4 border-white/5 bg-white/5 relative overflow-hidden flex flex-col ${isPriority ? 'h-[150px] md:h-[160px] p-5' : isCompact ? 'h-[140px] p-5' : 'h-[220px] p-6'}`}>
                    <div className="absolute inset-0 bg-gradient-to-r from-transparent via-white/5 to-transparent -translate-x-full animate-[shimmer_2s_infinite]"></div>
                    <div className={`w-8 h-8 rounded-full bg-white/5 ${isPriority ? 'mb-auto' : 'absolute top-4 right-4'}`}></div>
                    <div className={`${isPriority ? '' : 'pr-8 mb-4'}`}>
                        <div className={`h-6 bg-white/10 rounded-lg w-3/4 mb-3 ${isPriority ? 'mt-2' : ''}`}></div>
                        {!isPriority && <div className="space-y-2">
        ...
        const SkeletonQuickTask = () => (
            <div className="animate-pulse flex items-center gap-4 p-4 glass-panel rounded-2xl border border-white/5 relative overflow-hidden">
                <div className="absolute inset-0 bg-gradient-to-r from-transparent via-white/5 to-transparent -translate-x-full animate-[shimmer_2s_infinite]"></div>
        ...
        /* LABEL: PRIORITY-NOTE-SKELETON â€” Loading state for Priority Note cards */
        /* NEW 2026-04-16: Matches NoteCard variant="priority" dimensions for smooth transition */
        const PriorityNoteSkeleton = ({ index }) => {
            return (
                <div className={`skeleton-priority-card sticky-note-${(index % 5) + 1} overflow-hidden shadow-lg`}>
                    <div className="w-8 h-8 rounded-xl shimmer opacity-20 mb-4"></div>
                    <div className="mt-auto space-y-2">
                        <div className="w-12 h-2 rounded shimmer opacity-10"></div>
                        <div className="w-3/4 h-5 rounded-lg shimmer opacity-20"></div>
                    </div>
                </div>
            );
        };
                ...
                return (
                    ...
                    >
                        ...
                        <div className="mt-auto w-full flex flex-col gap-1 text-left">
                            <p className={`text-[9px] font-extrabold uppercase tracking-[0.2em] font-montserrat ${theme.label}`}>PRIORITY</p>
                            <h3 className={`text-xl font-bold tracking-tight font-display truncate w-full ${theme.text}`}>{formatTitle(note.title) || "Untitled"}</h3>
                        </div>
                    </div>
                );
            }
        ...
        const QuickTaskItem = React.memo(({ task, onToggle, onDelete, onEdit }) => {
            ...
            return (
                ...
                >
                    ...
                    {/* LABEL: QUIK-TSK-ITM â€” Desktop/Mobile Task Item */}
        ...
        // ==========================================================================
        // LABEL: PAGE-HOME (Dashboard)
        // Path: /
        // Sections: Priority Notes, Quick Tasks
        // ==========================================================================
        const DashboardPage = ({ user, notes, quickTasks, alarms = [], onOpenCreator, onEditNote, onReorderPriorityNote, onToggleQuickTask, onAddQuickTaskClick, onDeleteQuickTask, onUpdateQuickTask, onEditQuickTask, editingQuickTask, setEditingQuickTask, isQuickTaskModalOpen, setIsQuickTaskModalOpen, onRemoveReminder, isProbing, isFirstSyncDone, pomodoroTime, isPomodoroActive, setIsPomodoroActive }) => {
            const navigate = useNavigate();
            ...
            const handlePointerMoveCore = (mx, my) => {
                ...
                // handlePointerMoveCore - Dashboard reorder logic sync [FIX 2026-04-16]
                    ...
                    <div className="max-w-7xl mx-auto w-full px-0 md:px-12 pt-0 pb-12">
                        ...
                        <section className="mt-20 md:mt-8 mb-12 md:mb-16 px-4 md:px-0">
                            ...
                            {(isProbing || !isFirstSyncDone) ? (
                                ...
                                queryParams.get('search') && sortedPriorityNotes.length === 0 ? (
                                    <div className="col-span-full py-20 text-center glass-panel rounded-3xl border border-white/5">
                                        ...
                                        <p className="text-[10px] uppercase tracking-widest font-bold text-white/30">No matching important notes</p>
                                    </div>
                                ) : (
                                    <div className="grid grid-cols-2 lg:grid-cols-6 gap-3 md:gap-6 px-0 md:px-2 mb-16 md:mb-20">
                                        {sortedPriorityNotes.slice(0, 6).map((note, index) => (
                                            ...
                                            >
                                                {/* note-card-container: individual priority note [FIX 2026-04-16] */}
                                                <NoteCard 
        ...
        // LABEL: PAGE-NOTES
        // Path: /notes
        // Includes: Pinned Notes, All Notes, Sections
        // ==========================================================================
        /* FIX 2026-04-16: Added isFirstSyncDone to NotesPage signature for synchronized skeleton lifecycle */
        const NotesPage = React.memo(({ user, notes, onOpenCreator, onEditNote, noteSections, onAddSection, onDeleteSection, onMoveNote, onReorderNote, onRemoveReminder, onBulkUpdate, onBulkDelete, isProbing, isFirstSyncDone, pomodoroTime, isPomodoroActive }) => {
            const navigate = useNavigate();
            ...
            // Close dropdown on outside click
             ...
             // Disable right-click on the notes page (but ALLOW on editable areas for mobile context menu)
             useEffect(() => {
                 const handler = (e) => {
                     // Allow context menu if target is an input, textarea or contenteditable
            ...
            // Gesture state ref â€” shared between pointer event handlers
            const gestureRef = useRef({ active: false, activated: false, scrolling: false });
            ...
            // Only onPointerDown is on individual cards.
            // Move/Up/Cancel are added on document ONLY when a gesture starts,
            // to avoid blocking native scroll with per-card handlers.
            ...
            const handlePointerDown = (noteId, e) => {
                ...
                // FIX 2026-04-15: Increased to 550ms (was 500ms) for clearer "hold-to-move" intent
                longPressTimer.current = setTimeout(() => {
                    if (navigator.vibrate) try { navigator.vibrate(50); } catch(err) {}
                    gestureRef.current.activated = true;
            ...
            // Core move logic (called from document listener, only during drag)
            const handlePointerMoveCore = (mx, my) => {
            ...
            const renderNoteGrid = (notesList, gridClass, currentSection = null) => {
                ...
                // User prefers uniform sizes matching the Anima Christi spacing rather than dense mode
                const isDense = false; 
                // Enforcing a uniform gap-8 across all notes, keeping grid configuration
                ...
                return (
                    <div className={activeGridClass}>
                        {notesList.map((note, index) => {
                            ...
                            if (isDragged) {
                                // Anchor: The original DOM element that must stay in its starting section to keep pointer capture.
                                // We keep it in the original section's grid regardless of where the note currently "is".
                                ...
                                return (
                                    <React.Fragment key={note.id}>
                                        {/* 1. SEPARATE ANCHOR: Persistent and invisible to hold pointer capture */}
                                        {isHomeSection && (
                                            <div 
                                        ...
                                        {/* 2. PLACEHOLDER: The visual destination feedback */}
                                        {isTargetSection && (
                            ...
                            {(searchQuery || labelFilter) && localNotes.filter(matchesFilter).length === 0 && (
                                ...
                                {(() => {
                                    ...
                                    const pinnedFiltered = localNotes.filter(n => {
                                        ...
                                        // Anchor: dragged from pinned grid but currently unpinned in localNotes (e.g. pinned -> unpinned drag)
                                    ...
                                    const customSections = (noteSections || []).map(sectionName => {
                                        const sNotes = localNotes.filter(n => {
                                            ...
                                            const matches = matchesFilter(n);
                                            // Normal: match section name
                                            if (n.section === sectionName) return matches;
                                            // Hidden Anchor: match initialSection if it's being dragged
                                    ...
                                    const allNotesFiltered = localNotes.filter(n => {
                                        if (n.isPinned) return false;
                                        // Normal logic: hidden if in a custom section
                                        if (noteSections.includes(n.section)) {
                                            // Anchor: dragged from All Notes grid but currently in a custom section
                                            if (dragState && dragState.noteId === n.id && (dragState.initialSection || "") === "") return true;
                                            return false;
                                        }
                                        const matches = matchesFilter(n);
                                        // Normal: No section or section not in noteSections
        ...
        const CalendarPage = ({ user, notes, quickTasks = [], onOpenCreator, onEditNote, onToggleQuickTask, onEditQuickTask, onAddQuickTask, pomodoroTime, isPomodoroActive }) => {
            ...
            /* 2026-04-15: Restored corrupted calendar grid population logic */
                            ...
                            <div className="px-4 md:px-0 mt-24 md:mt-10">
                                <div className="flex items-center justify-between mb-8 md:mb-12">
                                    <div className="flex items-center justify-between w-full">
                                        ...
                                        {/* FIX 2026-04-22: Month title is now perfectly centered between arrows as requested */}
        ...
        const QuickTasksPage = ({ user, quickTasks = [], onOpenCreator, onToggleQuickTask, onAddQuickTaskClick, onDeleteQuickTask, onEditQuickTask, isProbing, isFirstSyncDone, pomodoroTime, isPomodoroActive }) => {
            const location = useLocation();
            const queryParams = new URLSearchParams(location.search);
                ...
                <Layout onOpenCreator={onOpenCreator} onFabClick={onAddQuickTaskClick} pomodoroTime={pomodoroTime} isPomodoroActive={isPomodoroActive}>
                    <div className="max-w-6xl mx-auto w-full px-4 md:px-8 pt-0 pb-24">
                        ...
                        <div className="flex items-center gap-4 mt-24 md:mt-10 mb-10 md:mb-16">
                            {/* FIX 2026-04-22: Page Header style matched to homepage serif aesthetic (non-italic) for consistency */}
                            <h1 className="text-xl md:text-3xl font-display font-bold text-cream-light tracking-[0.15em] md:tracking-[0.25em] uppercase">Quick Tasks</h1>
                        ...
                        <div className="space-y-6">
                            ...
                            {(isProbing || !isFirstSyncDone) ? (
                                ...
                                <div className="glass-panel p-10 rounded-3xl text-center border-dashed border-white/5">
                                    <p className="text-white/20 text-sm font-bold uppercase tracking-widest">{searchQuery ? 'No tasks match your search' : 'No quick tasks yet'}</p>
                                </div>
                            ) : (
                                <div className="space-y-8">
        ...
        /* StatsPage Component: Visualizes productivity metrics */
        // ==========================================================================
        // LABEL: PAGE-STATS (Analysis)
        // Path: /stats
        // Includes: Usage charts and task completion data
        // ==========================================================================
        const StatsPage = ({ user, onOpenCreator, quickTasks = [], pomodoroTime, isPomodoroActive }) => {
            const stats = useMemo(() => {
                const all = Array.isArray(quickTasks) ? quickTasks : [];
                const completed = all.filter(t => t.completed);
                const pending = all.filter(t => !t.completed);
            ...
            return (
                ...
                <Layout onOpenCreator={onOpenCreator} showFab={false} pomodoroTime={pomodoroTime} isPomodoroActive={isPomodoroActive}>
                    {/* FIX 2026-04-15: Increased to pt-[110px] for better header separation */}
                    <div className="max-w-7xl mx-auto w-full px-0 md:px-12 pt-[110px] md:pt-10 pb-24">
                        <Header user={user} subtitle="Productivity Statistics" showSearch={false} />
                        <div className="px-4 md:px-0 space-y-6 md:space-y-8">
                            <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 md:gap-5">
                                <div className="glass-panel rounded-2xl p-4 md:p-5">
                                    <p className="text-[10px] uppercase tracking-widest text-cream-light/40 font-bold">Total Tasks</p>
                                    <p className="text-2xl md:text-3xl font-display text-cream-light mt-2">{stats.total}</p>
                                </div>
                                <div className="glass-panel rounded-2xl p-4 md:p-5">
                                    <p className="text-[10px] uppercase tracking-widest text-cream-light/40 font-bold">Completed</p>
                                    <p className="text-2xl md:text-3xl font-display text-green-300 mt-2">{stats.completed}</p>
                                </div>
                                <div className="glass-panel rounded-2xl p-4 md:p-5">
                                    <p className="text-[10px] uppercase tracking-widest text-cream-light/40 font-bold">Done Today</p>
                                    <p className="text-2xl md:text-3xl font-display text-primary mt-2">{stats.doneToday}</p>
                                </div>
                                <div className="glass-panel rounded-2xl p-4 md:p-5">
                                    <p className="text-[10px] uppercase tracking-widest text-cream-light/40 font-bold">Overdue</p>
                                    <p className="text-2xl md:text-3xl font-display text-red-300 mt-2">{stats.overdue}</p>
                                </div>
                            </div>

                            <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 md:gap-8">
                                <div className="lg:col-span-4 glass-panel rounded-3xl p-6 md:p-8 flex flex-col items-center justify-center card-glow">
                                    <h3 className="text-xl font-display font-bold text-cream-light italic mb-6 w-full">Overall Progress</h3>
                                    <div className="relative w-48 h-48 md:w-56 md:h-56 flex items-center justify-center">
                                        <svg className="w-full h-full -rotate-90" viewBox="0 0 100 100">
                                            <circle cx="50" cy="50" fill="transparent" r="42" stroke="rgba(255,255,255,0.08)" strokeWidth="8"></circle>
                                            <circle cx="50" cy="50" fill="transparent" r="42" stroke="#f97316" strokeDasharray={`${ringProgress} ${ringCirc}`} strokeLinecap="round" strokeWidth="8"></circle>
                                        </svg>
                                        <div className="absolute inset-0 flex flex-col items-center justify-center">
                                            <span className="text-4xl font-display font-bold text-cream-light">{stats.completionRate}%</span>
                                            <span className="text-[10px] font-bold text-primary/60 uppercase tracking-widest mt-1">Completed</span>
                                        </div>
                                    </div>
                                </div>

                                <div className="lg:col-span-8 glass-panel rounded-3xl p-6 md:p-8 card-glow">
                                    <h3 className="text-xl font-display font-bold text-cream-light italic mb-6">Weekly Momentum</h3>
                                    <div className="grid grid-cols-7 gap-2 md:gap-3 items-end h-44 md:h-52">
                                        {stats.weekBuckets.map(bucket => {
                                            const pct = Math.max(8, Math.round((bucket.done / weeklyMax) * 100));
                                            return (
                                                <div key={bucket.key} className="flex flex-col items-center justify-end h-full">
                                                    <div className="w-full rounded-xl bg-white/5 border border-white/5 overflow-hidden h-full flex items-end">
                                                        <div className="w-full bg-gradient-to-t from-primary to-orange-300/80 rounded-xl transition-all duration-300" style={{ height: `${pct}%` }}></div>
                                                    </div>
                                                    <p className="text-[9px] md:text-[10px] text-cream-light/50 mt-2 uppercase tracking-wider font-bold">{bucket.key}</p>
                                                    <p className="text-[10px] text-cream-light/70 font-bold">{bucket.done}</p>
                                                </div>
                                            );
                                        })}
                                    </div>
                            ...
                            <div className="glass-panel rounded-3xl p-6 md:p-8">
                                <h3 className="text-lg md:text-xl font-display font-bold text-cream-light italic mb-4">Insight</h3>
                                <p className="text-sm md:text-base text-cream-light/70 font-sans leading-relaxed">
                                    {stats.total === 0
                                        ? "Start by adding your first quick task. Your progress analytics will appear here automatically."
                                        : stats.completionRate >= 80
                                            ? "Excellent consistency. Keep your momentum by clearing overdue tasks first to maintain high completion."
                                            : stats.completionRate >= 50
                                                ? "Good pace. A quick win is finishing 2-3 pending tasks today to push your completion above 70%."
                                                : "You have room to recover. Focus on completing today's tasks first before adding new ones."}
                                </p>
                            </div>
                        </div>
                    </div>
        ...
        /* StreakPage Component: Displays gamification streaks and rewards */
        // ==========================================================================
        // LABEL: PAGE-STREAK
        // Path: /streak
        // Includes: Gamification, Daily Streaks, Badges
        // ==========================================================================
        const StreakPage = ({ user, onOpenCreator, gamification, pomodoroTime, isPomodoroActive }) => {
            ...
            return (
                ...
                <Layout onOpenCreator={onOpenCreator} showFab={false} pomodoroTime={pomodoroTime} isPomodoroActive={isPomodoroActive}>
                    {/* FIX 2026-04-15: Increased to pt-[110px] for better header separation */}
                    <div className="max-w-2xl mx-auto w-full pt-[108px] pb-28 px-5 md:px-0">
                        <Header user={user} subtitle="Streak & Achievements" showSearch={false} />
                        <div className="px-0 md:px-0 space-y-6">
                            <section className="glass-panel rounded-[2.25rem] p-6 md:p-8 flex flex-col items-center text-center">
                                ...
                                <h2 className="text-8xl font-black text-primary italic glow-orange tracking-tighter mb-2">{currentStreak} {streakUnit}</h2>
                                <p className="text-cream-light/40 text-lg uppercase tracking-[0.5em] font-sans font-bold">Current Streak</p>
                                <p className="text-cream-light/30 text-xs mt-2 uppercase tracking-wider">Best: {gamification?.longestStreak || 0} days</p>
                            </section>
                            <section className="grid grid-cols-1 md:grid-cols-3 gap-4 md:gap-6 mb-20">
                                {(rewards.length ? rewards : ['Login Starter Reward']).map((reward, idx) => (
                                    <div key={idx} className="glass-panel card-glow rounded-3xl p-6 md:p-8 flex flex-col items-center text-center gap-4">
                                        ...
                                        <h3 className="text-xl font-bold text-cream-light">{typeof reward === 'string' ? reward : reward.name}</h3>
                                        <p className="text-cream-light/40 text-sm font-sans">Daily login reward unlocked.</p>
                                    </div>
                                ))}
                            </section>
                        </div>
        ...
        const AlarmWheelColumn = React.memo(({ values, selected, onChange, label, infinite = false }) => {
            ...
            useEffect(() => {
                ...
                // FIX 2026-04-22: Added 100ms timeout to ensure the modal is fully rendered/visible before initial scroll sync.
                // This prevents the "misaligned/broken" UI on first opening.
                ...
                <div className="flex flex-col items-center relative overflow-hidden">
                    ...
                    >
                        ...
                        {displayValues.map((v, i) => {
                            ...
                            /* FIX 2026-04-22: Enhanced visibility for "ghost" numbers (non-active items) as requested */
                            return (
                                <div
                                    key={`item-${v}-${i}`}
        ...
        /* ClockPage Component: Displays analog clock and alarm management */
        // ==========================================================================
        //        // LABEL: ClockPage â€” Minimalist Samsung-style UI for clock, pomodoro, and alarms
        // FIX 2026-04-20: Redesigned for basic UI (Samsung-style), removed verbose descriptions, added edit mode
        const ClockPage = ({ user, onOpenCreator, alarms = [], onAddAlarm, onToggleAlarm, onDeleteAlarm, pomodoroTime, setPomodoroTime, isPomodoroActive, setIsPomodoroActive, pomodoroSessions, alarmOverlayPermission = false, onRequestAlarmOverlayPermission, onRefreshAlarmOverlayPermission, hasNativeAlarmBridge = false, alarmsOnly = false }) => {
            ...
            /* MOVED: getWaitTimeText 2026-04-20 â€” Now a global utility */
                ...
                <Layout onOpenCreator={onOpenCreator} showFab={true} onFabClick={handleStartAdd} pomodoroTime={pomodoroTime} isPomodoroActive={isPomodoroActive}>
                    <div className={`${alarmsOnly ? 'max-w-4xl' : 'max-w-7xl'} mx-auto w-full px-4 md:px-16 pt-0 pb-96`}>
                        ...
                        <div className="flex items-center mt-24 md:mt-10 mb-10 gap-4">
                            ...
                            {/* FIX 2026-04-22: Subtle Overlay Permission Indicator (Top Right) as requested */}
                            {hasNativeAlarmBridge && (
                                <button 
                        ...
                        <div id="faiora_clock_grid" className={alarmsOnly ? 'grid grid-cols-1 gap-10 items-start' : 'grid grid-cols-1 lg:grid-cols-2 gap-10 md:gap-16 items-start'}>
                            ...
                            {!alarmsOnly && <div id="faiora_clock_section" className="space-y-8">
                                <div id="faiora_clock_card" className="faiora-clock-card glass-panel rounded-[2.5rem] p-6 md:p-10 flex flex-col items-center justify-center space-y-10">
                                    ...
                                    <div className="text-center">
                                        <h2 className="text-6xl font-display text-cream-light mb-2">{now.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</h2>
                                        <p className="text-primary/60 font-sans tracking-[0.2em] uppercase text-sm font-bold">{now.toLocaleDateString([], { weekday: 'long', month: 'short', day: 'numeric' })}</p>
                                    </div>
                                </div>

                                {/* LABEL: Pomodoro Focus Card â€” Integrated timer for deep work sessions */}
                                {/* FIX 2026-04-20: Added label title for Pomodoro section as requested */}
                                ...
                                <div id="faiora_pomodoro_card" ref={pomodoroSectionRef} className="faiora-pomodoro-card glass-panel p-6 md:p-10 rounded-[2.5rem] flex flex-col items-center justify-center space-y-8 relative overflow-hidden group">
                                    <div id="faiora_pomodoro_timer_circle" className="relative w-64 h-64 flex items-center justify-center">
                                        ...
                                        <div className="absolute inset-0 flex flex-col items-center justify-center text-center">
                                            <div className={`text-6xl font-display font-bold text-cream-light tracking-tighter ${isPomodoroActive ? 'animate-pulse' : ''}`}>{Math.floor(pomodoroTime / 60)}:{String(pomodoroTime % 60).padStart(2, '0')}</div>
                                            <p className="text-[10px] font-bold text-cream-light/30 uppercase tracking-[0.2em] mt-2">Focus Mode</p>
                                        </div>
                                    </div>
                                    <div className="flex items-center gap-6 relative z-10">
                                        <button onClick={() => setIsPomodoroActive(!isPomodoroActive)} className={`w-16 h-16 flex items-center justify-center rounded-full transition-all duration-300 shadow-xl ${isPomodoroActive ? 'bg-rose-500/20 text-rose-400 border border-rose-500/30' : 'bg-primary/20 text-primary border border-primary/30'}`}>
                                            <span className="material-symbols-outlined text-4xl font-bold">{isPomodoroActive ? 'pause' : 'play_arrow'}</span>
                                        </button>
                                        <button onClick={() => { setIsPomodoroActive(false); setPomodoroTime(25 * 60); }} className="w-16 h-16 flex items-center justify-center rounded-full bg-white/5 text-slate-400 hover:text-slate-200 transition-all border border-white/5 hover:bg-white/10">
                            ...
                            <div className="space-y-6">
                                ...
                                {isFormOpen && (() => {
                                    // Parse current alarmTime for Wheels
                                            ...
                                            >
                                                <form onSubmit={handleSubmitAlarm} className="space-y-8">
                                                    <div className="flex justify-between items-center">
                                                        ...
                                                        <button type="button" onClick={handleCancelForm} className="w-10 h-10 flex items-center justify-center rounded-full bg-white/5 text-cream-light/20 hover:text-white transition-all">
                                                            <span className="material-symbols-outlined text-lg">close</span>
                                                        </button>
                                                    </div>
                                                    
                                                    {/* SAMSUNG WHEEL PICKER - Optimized for Perfect Centering */}
                                                    <div className="flex items-center justify-center gap-4 py-4 relative">
                                                        {/* Center Highlight Bar FIX 2026-04-20: Adjusted height for 4xl font */}
                                                        ...
                                                        <div className="w-2" /> {/* Refined spacer to keep AM/PM closer for a balanced look */}
                                                        <div className="w-20">
                                                            <AlarmWheelColumn 
                                                                values={ALARM_AMPM} 
                                                                selected={ampmVal} 
                                                    ...
                                                    <div className="space-y-8">
                                                        ...
                                                        {/* SNOOZE TOGGLE */}
                                                        <div className="flex items-center justify-between glass-panel p-5 rounded-[1.75rem] border-white/5 bg-white/[0.02]">
                                                            <div className="flex flex-col">
                                                    ...
                                                    <div className="flex gap-4 pt-2">
                                                        <button type="button" onClick={handleCancelForm} className="flex-1 py-5 rounded-[1.75rem] bg-white/5 text-cream-light/30 text-[10px] font-black uppercase tracking-[0.3em] hover:bg-white/10 transition-all border border-white/5">Cancel</button>
                                                        <button type="submit" className="flex-1 py-5 rounded-[1.75rem] bg-primary text-white text-[10px] font-black uppercase tracking-[0.3em] shadow-2xl shadow-primary/40 hover:bg-primary-dark transition-all active:scale-95">Save</button>
                                                    </div>
                                                </form>
                                            </div>
                                        </div>
                                ...
                                {/* Alarm List (Samsung Style) */}
                                <div className="space-y-3">
                                    {/* UPCOMING ALARM STATUS FIX 2026-04-20 */}
                                    {(() => {
                                    ...
                                    {sortedAlarms.length === 0 && !isFormOpen && (
                                        <div className="text-center py-12 text-cream-light/20">
        ...
        const EnhancedProfilePage = ({ user, onOpenCreator, profileData, onSaveProfile, quickTasks = [], gamification, pomodoroTime, setPomodoroTime, isPomodoroActive, setIsPomodoroActive, pomodoroSessions, activeCollection, cloudFieldsCount, showToast, isSyncHealthy }) => {
            const [name, setName] = useState(profileData?.name || user?.displayName || '');
            ...
            return (
                <Layout onOpenCreator={onOpenCreator} pomodoroTime={pomodoroTime} isPomodoroActive={isPomodoroActive}>
                    <div className="flex flex-col h-full bg-charcoal/50">
                        ...
                        <div className="flex-1 overflow-y-auto no-scrollbar pt-12 md:pt-20 px-4 md:px-12 pb-24">
                            <div className="max-w-7xl mx-auto w-full space-y-8">

                                <section className="grid grid-cols-1 xl:grid-cols-[1.5fr_0.5fr] gap-6">
                                    {/* FIX 2026-04-22: Redesigned Profile Card - Left Aligned Identity + Productivity Donut */}
                                    <div className="glass-panel rounded-[2.75rem] overflow-hidden relative border-white/5 shadow-2xl bg-charcoal/40 backdrop-blur-3xl">
                                        {/* Slim Hero Banner */}
                                        <div className="h-20 md:h-24 bg-gradient-to-r from-primary/40 via-primary/20 to-orange-500/10 relative">
                                            <div className="absolute inset-0 bg-[url('https://www.transparenttextures.com/patterns/carbon-fibre.png')] opacity-10"></div>
                                        </div>
                                        
                                        <div className="px-6 md:px-8 pb-8 -mt-12 md:-mt-14">
                                            <div className="flex flex-col md:flex-row items-center md:items-end justify-between gap-6">
                                                {/* Left Column: Avatar & Identity */}
                                                <div className="flex flex-col md:flex-row items-center md:items-end gap-6 flex-1 w-full">
                                                    <div className="w-24 h-24 md:w-32 md:h-32 rounded-[2rem] overflow-hidden border-4 border-charcoal bg-zinc-900 shadow-2xl shrink-0 group relative cursor-pointer" onClick={handleToggleEdit}>
                                                        {activeAvatarSrc && !imgError ? (
                                                            <img
                                                                key={activeAvatarSrc}
                                                                src={activeAvatarSrc}
                                                            ...
                                                            <div className="w-full h-full flex items-center justify-center bg-gradient-to-br from-primary to-burnt-orange shadow-inner text-white font-black text-3xl">
                                                                {(name || user?.displayName || 'U').split(' ').map(n => n[0]).join('').slice(0, 2).toUpperCase()}
                                                            </div>
                                                        )}
                                                    ...
                                                    <div className="flex-1 text-center md:text-left space-y-2 pb-1">
                                                        <p className="text-[10px] font-black text-primary tracking-[0.32em] uppercase">Level 12 Focus Master</p>
                                                        {isEditing ? (
                                                            <input 
                                                                value={name} 
                                                            ...
                                                            <h1 className="text-2xl md:text-4xl font-display text-cream-light tracking-tight truncate max-w-[300px]">{name || user?.displayName || 'Adventurer'}</h1>
                                                        )}
                                                        <div className="flex items-center justify-center md:justify-start gap-2">
                                                            <span className="material-symbols-outlined text-[14px] text-primary">bolt</span>
                                                            {isEditing ? (
                                                                <input value={status} onChange={(e) => setStatus(e.target.value)} className="bg-white/5 border-b border-white/10 text-[10px] uppercase font-bold tracking-widest text-cream-light/60 outline-none" />
                                                            ) : (
                                                                <p className="text-[10px] font-black uppercase tracking-[0.2em] text-cream-light/40 truncate">{status || 'Stay in Motion'}</p>
                                                            )}
                                                        </div>
                                                        {/* Bio integration: Simple and compact */}
                                                        {isEditing ? (
                                                            <textarea value={bio} onChange={(e) => setBio(e.target.value)} className="w-full mt-2 bg-white/5 border border-white/5 rounded-xl px-3 py-2 text-[10px] text-cream-light/60 outline-none" />
                                                        ) : (
                                                            bio && <p className="text-[10px] text-cream-light/30 italic line-clamp-1 mt-1 font-medium">{bio}</p>
                                                        )}
                                                    </div>
                                                </div>

                                                {/* Right Column: Productivity Donut */}
                                                <div className="hidden lg:flex flex-col items-center gap-3 glass-panel-dark p-4 rounded-3xl border-white/5 shadow-inner">
                                                    <div className="relative w-20 h-20">
                                                        ...
                                                        <div className="absolute inset-0 flex items-center justify-center">
                                                            <span className="text-sm font-display text-cream-light">{Math.round(stats.completionRate)}%</span>
                                                        </div>
                                                    </div>
                                                    <p className="text-[8px] font-black uppercase tracking-[0.2em] text-cream-light/30">Daily Focus</p>
                                                </div>
                                                ...
                                                <div className="flex flex-col gap-2">
                                                    ...
                                                    >
                                                        <span className="material-symbols-outlined text-lg">{isEditing ? 'check' : 'settings'}</span>
                                                    </button>
                                                </div>
                                            </div>

                                            {/* Simplified Stats row integrated into main card */}
                                            <div className="mt-8 pt-8 border-t border-white/5 grid grid-cols-3 gap-4">
                                                <div className="text-center md:text-left">
                                                    <p className="text-[8px] font-black uppercase tracking-[0.24em] text-cream-light/35 mb-1">Total Tasks</p>
                                                    <p className="text-lg font-display text-cream-light">{stats.total}</p>
                                                </div>
                                                <div className="text-center md:text-left">
                                                    <p className="text-[8px] font-black uppercase tracking-[0.24em] text-cream-light/35 mb-1">Completed</p>
                                                    <p className="text-lg font-display text-primary">{stats.completed}</p>
                                                </div>
                                                <div className="text-center md:text-left">
                                                    <p className="text-[8px] font-black uppercase tracking-[0.24em] text-cream-light/35 mb-1">Efficiency</p>
                                                    <p className="text-lg font-display text-orange-400">{stats.completionRate}%</p>
                                                </div>
                                            </div>
                                        </div>
                                    </div>

                            <div className="glass-panel rounded-[2.75rem] p-6 md:p-8 space-y-5">
                                <div>
                                    <p className="text-[10px] font-black text-primary tracking-[0.32em] uppercase">Live Clock</p>
                                    <h2 className="text-4xl font-display text-cream-light mt-2">{now.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</h2>
                                    <p className="text-[10px] font-bold uppercase tracking-[0.18em] text-cream-light/35 mt-2">{now.toLocaleDateString([], { weekday: 'long', month: 'short', day: 'numeric' })}</p>
                                </div>
                                <div className="pt-2 border-t border-white/5">
                                    <p className="text-[10px] font-black text-primary tracking-[0.32em] uppercase">Next Focus</p>
                                    <h2 className="text-2xl font-display text-cream-light mt-2">{nextDueLabel}</h2>
                                </div>
                                <div className="grid grid-cols-2 lg:grid-cols-2 gap-3">
                                    <div className="rounded-[1.75rem] border border-white/5 bg-white/[0.03] p-4 relative overflow-hidden group">
                                        <div className="flex justify-between items-start">
                                            <p className="text-[10px] font-black uppercase tracking-[0.18em] text-cream-light/35">Streak</p>
                                            <span className="material-symbols-outlined text-primary text-xl animate-pulse" style={{fontVariationSettings: '"FILL" 1'}}>local_fire_department</span>
                                        </div>
                                        <p className="text-3xl font-display text-primary mt-2">{currentStreak}</p>
                                    </div>
                                    <div className="rounded-[1.75rem] border border-white/5 bg-white/[0.03] p-4 relative overflow-hidden group">
                                        <div className="flex justify-between items-start">
                                            <p className="text-[10px] font-black uppercase tracking-[0.18em] text-cream-light/35">Best</p>
                                            <span className="material-symbols-outlined text-cream-light/20 text-xl" style={{fontVariationSettings: '"FILL" 1'}}>local_fire_department</span>
                                        </div>
                                        <p className="text-3xl font-display text-cream-light mt-2">{longestStreak}</p>
                                    </div>
                                    <div className="rounded-[1.75rem] border border-white/5 bg-white/[0.03] p-4">
                                        <p className="text-[10px] font-black uppercase tracking-[0.18em] text-cream-light/35">Pomodoro</p>
                                        <p className="text-3xl font-display text-primary mt-2">{pomodoroSessions}</p>
                                    </div>
                                    <div className="rounded-[1.75rem] border border-white/5 bg-white/[0.03] p-4">
                                        <p className="text-[10px] font-black uppercase tracking-[0.18em] text-cream-light/35">Rewards</p>
                                        <p className="text-3xl font-display text-cream-light mt-2">{rewards.length}</p>
                                    </div>
                                </div>
                            </div>
                        </section>

                        <section className="glass-panel rounded-[2.75rem] p-6 md:p-8 space-y-6">
                            <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
                                <div>
                                    <p className="text-[10px] font-black text-primary tracking-[0.32em] uppercase">Detailed Stats</p>
                                    <h2 className="text-3xl font-display text-cream-light mt-2">Your productivity snapshot</h2>
                                </div>
                                <p className="text-[11px] font-bold uppercase tracking-[0.18em] text-cream-light/35">Completion rate {stats.completionRate}%</p>
                            </div>

                            <div className="grid grid-cols-2 lg:grid-cols-6 gap-3 md:gap-4">
                                <div className="rounded-[1.8rem] border border-white/5 bg-white/[0.03] p-4"><p className="text-[10px] uppercase tracking-widest text-cream-light/35 font-bold">Total</p><p className="text-3xl font-display text-cream-light mt-2">{stats.total}</p></div>
                                <div className="rounded-[1.8rem] border border-white/5 bg-white/[0.03] p-4"><p className="text-[10px] uppercase tracking-widest text-cream-light/35 font-bold">Pending</p><p className="text-3xl font-display text-cream-light mt-2">{stats.pending}</p></div>
                                <div className="rounded-[1.8rem] border border-white/5 bg-white/[0.03] p-4"><p className="text-[10px] uppercase tracking-widest text-cream-light/35 font-bold">Completed</p><p className="text-3xl font-display text-primary mt-2">{stats.completed}</p></div>
                                <div className="rounded-[1.8rem] border border-white/5 bg-white/[0.03] p-4"><p className="text-[10px] uppercase tracking-widest text-cream-light/35 font-bold">Done Today</p><p className="text-3xl font-display text-primary mt-2">{stats.doneToday}</p></div>
                                <div className="rounded-[1.8rem] border border-white/5 bg-white/[0.03] p-4"><p className="text-[10px] uppercase tracking-widest text-cream-light/35 font-bold">Due Today</p><p className="text-3xl font-display text-cream-light mt-2">{stats.dueToday}</p></div>
                                <div className="rounded-[1.8rem] border border-white/5 bg-white/[0.03] p-4"><p className="text-[10px] uppercase tracking-widest text-cream-light/35 font-bold">Overdue</p><p className="text-3xl font-display text-rose-400 mt-2">{stats.overdue}</p></div>
                            </div>

                            <div className="grid grid-cols-1 xl:grid-cols-[0.9fr_1.1fr] gap-6">
                                <div className="rounded-[2.75rem] border border-white/5 bg-white/[0.03] p-6 lg:p-8 flex flex-col md:flex-row items-center gap-8 shadow-2xl relative overflow-hidden">
                                     {/* Background Decor */}
                                    <div className="absolute top-0 right-0 w-32 h-32 bg-primary/5 blur-[60px] rounded-full -mr-16 -mt-16"></div>
                                    
                                    <div className="relative w-40 h-40 shrink-0">
                                        <svg className="w-full h-full -rotate-90 filter drop-shadow-[0_0_15px_rgba(249,115,22,0.1)]" viewBox="0 0 100 100">
                                            <circle cx="50" cy="50" r="42" fill="none" stroke="currentColor" strokeWidth="12" className="text-white/5" />
                                            <circle 
                                                cx="50" cy="50" r="42" fill="none" stroke="currentColor" strokeWidth="12" strokeLinecap="round"
                                                strokeDasharray={2 * Math.PI * 42}
                                                strokeDashoffset={2 * Math.PI * 42 * (1 - (stats.completionRate / 100))}
                                                className="text-primary transition-all duration-1000 ease-out"
                                            />
                                        ...
                                        <div className="absolute inset-0 flex flex-col items-center justify-center">
                                            <span className="text-3xl font-display text-cream-light">{Math.round(stats.completionRate)}%</span>
                                            <p className="text-[8px] font-black uppercase tracking-[0.2em] text-primary/60 mt-1">Efficiency</p>
                                        </div>
                                    </div>

                                    <div className="flex-1 w-full space-y-4">
                                        <div className="flex items-center justify-between gap-4 border-b border-white/5 pb-2">
                                            <h3 className="text-xs font-black uppercase tracking-[0.24em] text-cream-light/60">Completion Breakdown</h3>
                                            <span className="text-[10px] font-bold text-primary bg-primary/10 px-3 py-1 rounded-full whitespace-nowrap flex-shrink-0">{stats.noDate} No-Date Tasks</span>
                                        </div>
                                        
                                        <div className="grid grid-cols-2 gap-4">
                                            <div className="space-y-1">
                                                <p className="text-[10px] font-bold text-cream-light/30 uppercase tracking-widest">Next Target</p>
                                                <p className="text-sm font-medium text-cream-light truncate max-w-[150px]">{stats.nextDueTask ? formatTaskText(stats.nextDueTask.text) : 'None'}</p>
                                            </div>
                                            <div className="space-y-1">
                                                <p className="text-[10px] font-bold text-cream-light/30 uppercase tracking-widest">Active Streak</p>
                                                <div className="flex items-center gap-1">
                                                    <span className="material-symbols-outlined text-xs text-primary" style={{fontVariationSettings: '"FILL" 1'}}>local_fire_department</span>
                                                    <p className="text-sm font-bold text-primary">{currentStreak} Days</p>
                                                </div>
                                            </div>
                                            <div className="space-y-1">
                                                <p className="text-[10px] font-bold text-cream-light/30 uppercase tracking-widest">Upcoming</p>
                                                <p className="text-sm font-medium text-cream-light">{stats.dueTomorrow} Tasks</p>
                                            </div>
                                            <div className="space-y-1">
                                                <p className="text-[10px] font-bold text-cream-light/30 uppercase tracking-widest">Personal Best</p>
                                                <p className="text-sm font-bold text-cream-light/60">{longestStreak} Days</p>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                                <div className="rounded-[2rem] border border-white/5 bg-white/[0.03] p-5">
                                    <div className="flex items-center justify-between mb-4">
                                        <h3 className="text-sm font-bold uppercase tracking-[0.24em] text-cream-light/55">Last 7 Days</h3>
                                        <span className="text-[10px] font-bold uppercase tracking-[0.18em] text-primary/60">Completed tasks</span>
                                    </div>
                                    <div className="grid grid-cols-7 gap-3 items-end h-36">
                                        {stats.weekBuckets.map(bucket => {
                                            ...
                                            return (
                                                <div key={bucket.key} className="flex flex-col items-center gap-3 h-full justify-end">
                                                    <div className="w-full rounded-t-2xl bg-gradient-to-t from-primary to-orange-300/80" style={{ height }}></div>
                                                    <span className="text-[10px] font-bold uppercase tracking-[0.16em] text-cream-light/35">{bucket.key}</span>
                                                </div>
                                            );
                                        })}
                                    </div>
                        ...
                        <section className="grid grid-cols-1 xl:grid-cols-[0.9fr_1.1fr] gap-6">
                            <div className="glass-panel rounded-[2.75rem] p-6 md:p-8">
                                <div className="flex items-center justify-between">
                                    <div>
                                        <p className="text-[10px] font-black text-primary tracking-[0.32em] uppercase">Pomodoro</p>
                                        <h2 className="text-3xl font-display text-cream-light mt-2">Stay in motion</h2>
                                    </div>
                                    <span className="text-[10px] font-bold text-primary bg-primary/10 px-3 py-1 rounded-full uppercase tracking-[0.2em] border border-primary/20">{pomodoroSessions} Sessions</span>
                                </div>
                                ...
                                <div className="mt-8 flex flex-col items-center text-center">
                                    <div className="relative w-56 h-56">
                                        <svg className="w-full h-full -rotate-90" viewBox="0 0 120 120">
                                            ...
                                            <circle cx="60" cy="60" r="44" fill="none" stroke="url(#profilePomoGradient)" strokeWidth="8" strokeLinecap="round" strokeDasharray={pomodoroCirc} strokeDashoffset={pomodoroCirc - (pomodoroCirc * pomodoroProgress)} />
                                            <defs>
                                                <linearGradient id="profilePomoGradient" x1="0%" y1="0%" x2="100%" y2="100%">
                                        ...
                                        <div className="absolute inset-0 flex flex-col items-center justify-center">
                                            <div className={`text-5xl font-display text-cream-light tracking-tight ${isPomodoroActive ? 'animate-pulse' : ''}`}>{Math.floor(pomodoroTime / 60)}:{String(pomodoroTime % 60).padStart(2, '0')}</div>
                                            <p className="text-[10px] font-bold text-cream-light/35 uppercase tracking-[0.24em] mt-2">Focus Timer</p>
                                        </div>
                                    </div>

                                    <div className="flex items-center gap-4 mt-8">
                                        <button onClick={() => setIsPomodoroActive(!isPomodoroActive)} className={`w-16 h-16 rounded-full flex items-center justify-center border transition-all ${isPomodoroActive ? 'bg-rose-500/10 border-rose-500/20 text-rose-400' : 'bg-primary/10 border-primary/30 text-primary'}`}>
                                            <span className="material-symbols-outlined text-3xl">{isPomodoroActive ? 'pause' : 'play_arrow'}</span>
                                        </button>
                                        <button onClick={() => { setIsPomodoroActive(false); setPomodoroTime(25 * 60); localStorage.setItem('faiora_pomo_time', String(25 * 60)); }} className="w-16 h-16 rounded-full flex items-center justify-center border border-white/10 bg-white/5 text-cream-light/45 hover:text-cream-light transition-all">
                                            <span className="material-symbols-outlined text-3xl">refresh</span>
                                        </button>
                                    </div>
                                </div>
                            ...
                            <div className="glass-panel rounded-[2.75rem] p-6 md:p-8 space-y-5">
                                <div>
                                    <p className="text-[10px] font-black text-primary tracking-[0.32em] uppercase">Achievements</p>
                                    <h2 className="text-3xl font-display text-cream-light mt-2">Streaks and rewards</h2>
                                </div>
                                {/* FIX 2026-04-22: Removed redundant streak boxes to save space as requested */}
                                <div className="space-y-3">
                                    {rewards.map((reward, idx) => (
                                        <div key={`${reward}-${idx}`} className="rounded-[1.6rem] border border-white/5 bg-white/[0.03] px-4 py-4 flex items-center gap-4">
                                            <div className="w-12 h-12 rounded-2xl bg-primary/10 text-primary flex items-center justify-center">
                                                <span className="material-symbols-outlined" style={{fontVariationSettings: '"FILL" 1'}}>military_tech</span>
                                            </div>
                                            <div>
                                                <p className="text-sm font-bold text-cream-light">{typeof reward === 'string' ? reward : reward.name}</p>
                                                <p className="text-[10px] font-bold uppercase tracking-[0.18em] text-primary/60 mt-1">Unlocked reward</p>
                                            </div>
                                        </div>
                                    ))}
                                </div>
        ...
        // Path: /profile
        // ==========================================================================
        const ProfilePage = ({ user, onOpenCreator, profileData, onSaveProfile, pomodoroTime, isPomodoroActive, activeCollection, cloudFieldsCount }) => {
            const [name, setName] = useState(profileData?.name || user?.displayName || '');
            useEffect(() => {
                setName(profileData?.name || user?.displayName || '');
            }, [profileData, user]);
            return (
                <Layout onOpenCreator={onOpenCreator} pomodoroTime={pomodoroTime} isPomodoroActive={isPomodoroActive}>
                    {/* FIX 2026-04-15: Added pt-20 for mobile absolute-top header */}
                    <div className="max-w-3xl mx-auto w-full px-4 md:px-8 pt-20 md:pt-12 pb-12">
                        <Header user={user} subtitle="Profile" />
                        <div className="glass-panel rounded-3xl p-6 space-y-4">
                            <label className="text-xs uppercase tracking-widest text-cream-light/40">Display Name</label>
                            <input value={name} onChange={(e) => setName(e.target.value)} className="w-full bg-white/5 border border-white/10 rounded-xl px-3 py-2 text-cream-light" />
                            <button onClick={() => onSaveProfile({ name: name.trim() })} className="bg-primary hover:bg-primary-dark text-white px-4 py-2 rounded-xl text-xs font-bold uppercase tracking-wider">Save Profile</button>
                            <div className="pt-6 border-t border-white/5 flex flex-col items-center gap-1">
                                <p className="text-[10px] font-bold text-cream-light/20 uppercase tracking-[0.2em] font-montserrat">Account UID</p>
                                <div className="flex items-center gap-2 justify-center w-full mb-4">
                                    <p className="text-[10px] font-mono text-white/30 truncate max-w-[200px]">{user?.uid}</p>
                                    <button 
                                        onClick={() => {
                                ...
                                <p className="text-[10px] font-bold text-cream-light/20 uppercase tracking-[0.2em] font-montserrat">Active Database</p>
                                <p className="text-xs font-bold text-primary font-montserrat tracking-widest uppercase">{activeCollection || 'Not Connected'}</p>
                                
                                <p className="text-[10px] font-bold text-cream-light/20 uppercase tracking-[0.2em] font-montserrat mt-4">Database Structure</p>
                                <p className="text-xs font-bold text-primary/60 font-montserrat tracking-widest uppercase mb-4">{cloudFieldsCount || 0} Fields Found</p>

                                <div className="mt-4 flex flex-col items-center gap-1">
                                    <p className="text-[10px] font-bold text-cream-light/20 uppercase tracking-[0.2em] font-montserrat">Consensus Sync</p>
                                    <div className="flex gap-2">
                                        <button 
                                            onClick={() => {
                                    ...
                                    <p className={`text-[8px] font-bold uppercase tracking-widest mt-2 ${isSyncHealthy ? 'text-green-500/50' : 'text-red-500'}`}>
                                        Network Health: {isSyncHealthy ? 'HEALTHY' : 'UNSTABLE / STUCK'}
                                    </p>
                                </div>

                                <div className="mt-8 flex flex-col items-center gap-1 opacity-50">
                                    <p className="text-[10px] font-bold text-cream-light/20 uppercase tracking-[0.2em] font-montserrat">Faiora Version</p>
                                    <p className="text-xs font-bold text-primary/40 font-montserrat tracking-widest">v1.1.0-APK_STABLE</p>
                                </div>
                            </div>
                        </div>
                    </div>
        ...
        const SettingsPage = ({ user, onOpenCreator, settingsData, onSaveSettings, pomodoroTime, isPomodoroActive, showToast }) => {
            ...
            return (
                <Layout onOpenCreator={onOpenCreator} pomodoroTime={pomodoroTime} isPomodoroActive={isPomodoroActive}>
                    <div className="max-w-3xl mx-auto w-full px-4 md:px-8 pt-20 md:pt-12 pb-12">
                        <Header user={user} subtitle="Settings" />
                        <div className="space-y-6">
                            <div className="glass-panel rounded-3xl p-6 space-y-4">
                            ...
                            <div className="glass-panel rounded-3xl p-6 space-y-4">
                                <h3 className="text-xs font-bold text-primary uppercase tracking-widest mb-2">Data Management</h3>
                                <p className="text-xs text-cream-light/40 mb-4">Manage your deleted notes and recovery settings.</p>
                                <button 
                                    onClick={() => navigate('/trash')}
                                    className="flex items-center justify-between w-full p-4 bg-white/5 hover:bg-white/10 rounded-2xl transition-all group"
                                >
                                    <div className="flex items-center gap-3">
                                        <span className="material-symbols-outlined text-rose-400 group-hover:rotate-12 transition-transform">delete</span>
                                        <span className="text-sm font-medium text-cream-light/90">Trash Folder</span>
                                    </div>
                                ...
                                <button 
                                    onClick={async () => {
                                        if (window.confirm("Deep Scan will thoroughly verify all backup collections for missing notes. This may take a few moments. Proceed?")) {
                                            ...
                                            showToast("Deep scan complete. Restarting sync...");
                                ...
                                >
                                    <div className="flex items-center gap-3">
                                        ...
                                        <span className="text-sm font-bold text-primary italic">Repair Sync & Deep Scan</span>
                                    </div>
        ...
        /* TrashPage Component: View and recover deleted notes */
        // ==========================================================================
        // LABEL: PAGE-TRASH
        // Path: /trash
        // ==========================================================================
        const TrashPage = ({
            ...
            const [pendingAction, setPendingAction] = useState(null); // { type: 'restore'|'delete', itemType: 'note'|'task', id: string, title: string }
            ...
            const sortedTrashNotes = useMemo(() => (
                [...(trashNotes || [])].sort((a, b) => new Date(b.deletedAt || 0) - new Date(a.deletedAt || 0))
            ), [trashNotes]);
            const sortedTrashQuickTasks = useMemo(() => (
                [...(trashQuickTasks || [])].sort((a, b) => new Date(b.deletedAt || 0) - new Date(a.deletedAt || 0))
            ), [trashQuickTasks]);

            const renderCountdownChip = (item, accent = 'text-amber-300 border-amber-300/20 bg-amber-500/10') => (
                <span className={`inline-flex items-center gap-1 rounded-full px-2.5 py-1 text-[10px] font-bold uppercase tracking-[0.18em] ${accent}`}>
            ...
            return (
                <Layout onOpenCreator={onOpenCreator} showFab={false} pomodoroTime={pomodoroTime} isPomodoroActive={isPomodoroActive}>
                    <div className="max-w-7xl mx-auto w-full px-4 md:px-16 pt-[110px] md:pt-12 pb-12">
                        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
                            <Header user={user} subtitle="Trash" />
                            {totalTrashCount > 0 && (
                                <button 
                        ...
                        {totalTrashCount === 0 ? (
                            <div className="flex flex-col items-center justify-center py-20 text-center">
                                <div className="w-24 h-24 bg-white/5 rounded-full flex items-center justify-center mb-6">
                                    <span className="material-symbols-outlined text-4xl text-slate-600">delete_outline</span>
                                </div>
                                <h3 className="text-xl font-bold text-cream-light mb-2">Trash is empty</h3>
                                <p className="text-sm text-slate-500 max-w-xs">Deleted notes and quick tasks stay here for 30 days before they are permanently removed.</p>
                            </div>
                        ) : (
                            <div className="space-y-12">
                                <section className="space-y-6">
                                    <div className="flex items-center gap-3">
                                        ...
                                        <h3 className="text-sm font-bold text-cream-light/90 uppercase tracking-[0.3em]">Notes</h3>
                                        <span className="rounded-full bg-white/5 px-2.5 py-1 text-[10px] font-bold text-white/40 uppercase tracking-[0.18em]">{sortedTrashNotes.length}</span>
                                        <div className="h-px flex-1 bg-gradient-to-r from-primary/20 to-transparent"></div>
                                    </div>
                                    
                                    {sortedTrashNotes.length === 0 ? (
                                        <div className="glass-panel rounded-2xl p-6 text-sm text-white/35 border-dashed border-white/10">No deleted notes right now.</div>
                                    ) : (
                                        <div className="flex flex-col gap-2">
                                            {sortedTrashNotes.map(note => (
                                                <div key={note.id} className="glass-panel p-4 rounded-2xl flex items-center justify-between gap-4 group border border-white/5 hover:border-primary/20 transition-all">
                                                    <div className="flex items-center gap-4 min-w-0">
                                                        {/* trash_note_icon â€” Short Summary: Note's custom icon, consistent with dashboard */}
                                                        <div className="w-10 h-10 bg-white/5 rounded-xl flex items-center justify-center text-primary shrink-0">
                                                            <span className="material-symbols-outlined text-xl">{note.noteIcon || 'description'}</span>
                                                        </div>
                                                        <div className="min-w-0">
                                                            <h4 className="font-bold text-cream-light/90 truncate leading-tight">{note.title || 'Untitled'}</h4>
                                                            <p className="text-[9px] font-bold text-white/20 uppercase tracking-[0.15em] mt-1 text-slate-500">Deleted {new Date(note.deletedAt).toLocaleDateString()}</p>
                                                        </div>
                                                    </div>

                                                    <div className="flex items-center gap-6 shrink-0">
                                                        <div className="hidden sm:block">
                                                            {renderCountdownChip(note)}
                                                        </div>
                                ...
                                <section className="space-y-6">
                                    <div className="flex items-center gap-3">
                                        ...
                                        <h3 className="text-sm font-bold text-cream-light/90 uppercase tracking-[0.3em]">Quick Tasks</h3>
                                        <span className="rounded-full bg-white/5 px-2.5 py-1 text-[10px] font-bold text-white/40 uppercase tracking-[0.18em]">{sortedTrashQuickTasks.length}</span>
                                        <div className="h-px flex-1 bg-gradient-to-r from-amber-400/20 to-transparent"></div>
                                    </div>

                                    {sortedTrashQuickTasks.length === 0 ? (
                                        <div className="glass-panel rounded-2xl p-6 text-sm text-white/35 border-dashed border-amber-300/10">No deleted quick tasks right now.</div>
                                    ) : (
                                        <div className="flex flex-col gap-2">
                                            {sortedTrashQuickTasks.map(task => (
                                                <div key={task.id} className="glass-panel p-4 rounded-2xl flex items-center justify-between gap-4 group border border-white/5 hover:border-amber-300/20 transition-all">
                                                    <div className="flex items-center gap-4 min-w-0">
                                                        {/* trash_task_icon â€” Short Summary: Amber icon to distinguish tasks from notes */}
                                                        <div className="w-10 h-10 bg-amber-500/10 rounded-xl flex items-center justify-center text-amber-300 shrink-0">
                                                    ...
                                                    <div className="flex items-center gap-6 shrink-0">
                                                        <div className="hidden sm:block">
                                                            {renderCountdownChip(task, 'text-amber-300 border-amber-300/20 bg-amber-500/10')}
                                                        </div>
                                                        <div className="flex items-center gap-1 border-l border-white/10 pl-4">
                                                            <button 
                        ...
                        {pendingAction && (
                            <ConfirmationModal 
                                ...
                                message={pendingAction.type === 'restore' 
                                    ? `Are you sure you want to restore "${pendingAction.title}"? It will be moved back to your main collection.` 
                                    : `Are you sure you want to permanently delete "${pendingAction.title}"? This action cannot be undone.`
                                }
                                onConfirm={() => {
        ...
        const LoadingPage = ({ onComplete }) => {
            useEffect(() => {
                // FIX 2026-04-15: Reduced artificial loading delay from 1s to 200ms
                const timer = setTimeout(onComplete, 200);
                return () => clearTimeout(timer);
            }, [onComplete]);

            return (
                <div className="faiora-loading-screen fixed inset-0 z-50 blur-overlay flex flex-col items-center justify-center">
                    <div className="loading-arc-container relative flex items-center justify-center w-[252px] h-[252px]">
                        ...
                        <div className="faiora-loading-brand absolute flex flex-col items-center text-center z-10">
                            <div className="relative">
                                ...
                                <h1 className="faiora-loading-title text-7xl font-display font-bold tracking-tight text-cream-light italic leading-none drop-shadow-[0_0_30px_rgba(249,115,22,0.4)]">
                                    Faiora
                                </h1>
                            </div>
                            <div className="faiora-loading-copy mt-8">
                                <p className="text-primary text-[10px] uppercase tracking-[0.8em] font-sans font-bold">Fallo ora</p>
                            </div>
                        </div>
                    </div>
                    <div className="faiora-loading-progress-wrap absolute bottom-24 w-full px-12 max-w-md">
        ...
        // --- Notification Permission Banner (for mobile user-gesture requirement) ---
        const NotificationBanner = ({ user }) => {
            ...
            return (
                <div className="faiora-mobile-banner fixed top-0 left-0 right-0 z-[300] px-4 pt-3 pb-0 md:flex md:justify-center pointer-events-none" style={{paddingTop: 'env(safe-area-inset-top, 12px)'}}>
                    <div className="pointer-events-auto w-full md:max-w-lg bg-gradient-to-r from-orange-950/95 via-black/95 to-orange-950/95 backdrop-blur-2xl border border-orange-500/30 rounded-2xl p-4 shadow-[0_8px_40px_rgba(249,115,22,0.25)] flex items-center gap-3 animate-in slide-in-from-top-4 duration-500">
                        <div className="w-10 h-10 rounded-xl bg-orange-500/15 flex items-center justify-center shrink-0">
                            <span className="material-symbols-outlined text-orange-400 text-xl" style={{fontVariationSettings: '"FILL" 1'}}>notifications_active</span>
                        </div>
                        <div className="flex-1 min-w-0">
                            <p className="text-cream-light text-sm font-bold leading-tight">Enable Notifications</p>
                            <p className="text-white/50 text-[11px] font-sans mt-0.5 leading-snug">Get task reminders and alarm alerts on this device</p>
                        </div>
                        <button
        ...
        /* TransitionManager - handles page navigation transitions with fire-wipe overlay and content dissolve */
        /* FIX 2026-04-15: Added fade-in/fade-out dissolve animation on route content behind the fire sprite */
        const TransitionManager = ({ children, onTransitionStart, onTransitionEnd }) => {
            const FIRE_WIPE_DURATION_MS = 1400; // FIX 2026-04-22: Increased by 0.2s for a slightly smoother feel as requested
            const location = useLocation();
            ...
            const startTransition = (newLoc) => {
                ...
                /* t1: At 450ms, swap route content but keep it hidden (still dissolve-out) */
                const t1 = setTimeout(() => {
                    setDisplayLocation(normalizeRouteLocation(newLoc));
                }, 450); // FIX 2026-04-15: Reduced from 850ms

                /* t2: At 750ms, start the smooth fade-in as fire sprite begins to clear */
                /* Fire sprite peak is faster now, so content appears earlier for snappier feel */
                const t2 = setTimeout(() => {
                    setIsDissolving(false);
                    setIsFadingIn(true);
                }, 750); // FIX 2026-04-15: Reduced from 1300ms

                /* t3: Re-enable nav bar faster (at 600ms instead of 1100ms) */
                /* This eliminates the 'stuck' feeling during the fire sweep on mobile */
                const t3 = setTimeout(() => {
                    if (onTransitionEnd) setTimeout(() => onTransitionEnd(), 0);
                }, 600); // FIX 2026-04-15: Reduced from 1100ms

                /* t4: Fire sprite ends at 1200ms, remove fire overlay */
                const t4 = setTimeout(() => {
                    setIsWiping(false);
                }, FIRE_WIPE_DURATION_MS);

                /* t5: At 1500ms, fade-in animation done, cleanup dissolve state */
                const t5 = setTimeout(() => {
                    setIsFadingIn(false);
                }, 1500); // FIX 2026-04-15: Reduced from 2200ms
                
                timers.current = [t1, t2, t3, t4, t5];
            };

            useEffect(() => {
                // Do not run startTransition on first mount: it applies dissolve-out (opacity: 0) to all
                // routes until timers fire, which looks like a blank screen right after login when HashRouter mounts.
            ...
            useEffect(() => {
                const safety = setTimeout(() => {
                    ...
                    setIsWiping(false);
                    setIsFadingIn(false);
                }, 1600); // FIX 2026-04-15: Reduced from 2500ms safety limit
            ...
            /* route-transition-shell - dissolve class determines fade-out/fade-in state */
        ...
        const QuickTaskModal = ({ onClose, onAdd, initialData = null, showToast, prefillDate = null }) => {
            ...
            const [selectedPreset, setSelectedPreset] = useState(() => {
                ...
                if (prefillDate) return 'custom'; // though prefillDate implies custom usually
                return 'later';
            });
            ...
            useEffect(() => {
                if (!isClosing) {
                    ...
                    // If the modal is closing, and we previously pushed a state, go back
            ...
            const getSelectedLabel = () => {
                ...
                // Use logic to determine which preset to show if possible
                ...
                // Fallback smart matching if preset is somehow lost or doesn't match
            ...
            /* quick-task-modal - z-index raised above header */
            /* quick-task-modal - raised z-index to 999 */
            /* FIX 2026-04-15: Raised to z-[999] (was 500) */
            return (
                <div className={"fixed inset-0 z-[999] flex items-center justify-center p-4 transition-all duration-200 " + (isClosing ? 'opacity-0 scale-95' : 'animate-in fade-in duration-300')}>
                    <style>{`
                    ...
                    <div className="bg-[#0f172a] w-full max-w-sm rounded-[2.5rem] p-6 relative z-10 shadow-2xl border border-white/5 animate-in zoom-in-95 duration-300">
                        <div className="flex justify-between items-center mb-8">
                            ...
                            <button onClick={handleClose} className="text-white/20 hover:text-primary transition-colors">
                        ...
                        <form onSubmit={handleSubmit} className="space-y-6">
                            ...
                            <div className="space-y-3 relative" ref={dropdownRef}>
                                ...
                                {isDropdownOpen && (
                                    <div className="absolute top-full left-0 right-0 mt-2 bg-[#1e293b] border border-white/10 rounded-2xl shadow-2xl overflow-hidden z-50 animate-in fade-in slide-in-from-top-2 duration-200">
                                         {[
        ...
        const SetLockModal = ({ onSet, onClose, showToast }) => {
            ...
            useEffect(() => {
                const handleKeyDown = (e) => {
                    if (step === 3) return; // Let the input field handle its own events
            ...
            /* set-lock-modal - raised z-index to 999 */
            /* FIX 2026-04-15: Raised to z-[999] (was 500) */
            return (
                <div className={"fixed inset-0 z-[999] flex items-center justify-center p-4 transition-all duration-200 " + (isClosing ? 'opacity-0 scale-95' : 'animate-in fade-in zoom-in-95 duration-300')}>
                    <style>{`
                        @keyframes shake {
                    ...
                    <div className="bg-[#0f172a] w-full max-w-sm rounded-[2.5rem] p-8 relative z-[210] shadow-2xl border border-white/5">
                        ...
                        {step < 3 ? (
                            ...
                            <div className="flex flex-col gap-3">
                                <button onClick={handleSubmit} className="w-full bg-primary text-white py-4 rounded-2xl font-bold uppercase tracking-widest text-xs hover:bg-primary-dark transition-all">
                                    Set Lock
                                </button>
                                <button onClick={() => { setHint(""); setStep(3); handleSubmit(); }} className="w-full bg-white/5 text-slate-400 py-4 rounded-2xl font-bold uppercase tracking-widest text-xs hover:bg-white/10 transition-all">
                                    Skip Hint
                                </button>
                            </div>
                        )}
        ...
        const UnlockModal = ({ hint, onUnlock, onClose, showToast }) => {
            const [pin, setPin] = useState("");
            const [isClosing, setIsClosing] = useState(false);
            ...
            return (
                <div className={"fixed inset-0 z-[999] flex items-center justify-center p-4 transition-all duration-200 " + (isClosing ? 'opacity-0 scale-95' : 'animate-in fade-in zoom-in-95 duration-300')}>
                    <div className="absolute inset-0 bg-black/80 backdrop-blur-md" onClick={handleClose}></div>
                    <div className={`bg-[#0f172a] w-full max-w-sm rounded-[2.5rem] p-8 relative z-[210] shadow-2xl border border-white/5 ${isError ? 'animate-[shake_0.5s_ease-in-out]' : ''}`}>
                        <div className="text-center mb-8">
                            <div className="w-16 h-16 bg-primary/20 rounded-2xl flex items-center justify-center mx-auto mb-4">
                            ...
                            {hint && <p className="text-primary/60 text-[10px] font-bold uppercase tracking-widest mt-4">Hint: {hint}</p>}
                        </div>
        ...
        // ==========================================================================
        // LABEL: ALARM-OVERLAY-SYSTEM â€” Samsung-inspired alarm dismissal
        // ==========================================================================
        const SamsungAlarmOverlay = ({ alarm, onDismiss }) => {
            ...
            return (
                <div id="faiora_alarm_ringing_overlay" className="fixed inset-0 z-[2000] bg-black flex flex-col items-center justify-start overflow-hidden">
                    ...
                    {/* Ringing Info - Moved lower and text sized up */}
                    <div className="relative text-center mt-32 z-10">
                        {/* overlay_status_label â€” Short Summary: Standard 'Alarm Ringing' title */}
                    ...
                    {/* Samsung-Style Interaction Area - Repositioned for one-handed reach */}
                    <div className="absolute bottom-24 left-0 right-0 flex flex-col items-center z-20">
                        <div className="relative flex items-center justify-center h-72 w-full">
                            ...
                            >
                                <span className={`material-symbols-outlined text-5xl transition-all duration-300 ${isTargetReached ? 'text-white' : 'text-primary'}`}>
                                    {isTargetReached ? 'check' : 'close'}
                                </span>
                            </div>
                            ...
                            {!isDragging && (
                                <div className="absolute -bottom-12 flex flex-col items-center gap-2 opacity-30">
                                    ...
                                    <p className="text-[10px] font-bold text-white uppercase tracking-[0.3em]">Swipe to Stop</p>
                                </div>
                            )}
                        </div>
                    </div>
        ...
        const PermissionModal = ({ onGrant, onSkip }) => (
            <div className="permission-sheet-overlay">
                <div className="permission-sheet">
                    ...
                    <div className="flex flex-col items-center text-center">
                        <div className="flex h-12 w-12 items-center justify-center rounded-xl bg-primary/10 text-primary mb-4 ring-1 ring-primary/20">
                            <span className="material-symbols-outlined text-2xl">notification_important</span>
                        </div>
                        <h3 className="text-lg font-display font-medium text-cream-light mb-2 italic">Enable Overlay Access</h3>
                        <p className="text-[13px] text-white/40 leading-relaxed mb-6 max-w-[280px]">
                            Faiora needs permission to "Display over other apps" to ensure your alarms ring reliably.
                        </p>
                    </div>
        ...
        // ==========================================================================
        // LABEL: APP-CORE â€” Main application component
        ...
        const App = () => {
            ...
            const refreshAlarmOverlayPermission = useCallback(() => {
                ...
                setAlarmOverlayPermission(granted);
                return granted;
            }, []);
            ...
            useEffect(() => {
                return FaioraNotifications.subscribeInApp((alert) => {
                    if (!alert) return;
                    const normalized = {
                        id: alert.id || (Date.now() + Math.random()),
                        title: alert.title || 'Faiora Alert',
                        body: alert.body || '',
            ...
            useEffect(() => {
                ...
                // FIX 2026-04-20: Auto-prompt for overlay permission if not granted and haven't skipped yet
            ...
            const scheduleAlarm = useCallback((alarm) => {
                ...
                const timeoutId = setTimeout(() => {
                    ...
                    if (auth.currentUser) {
                        ...
                        if (activeCollection) {
                            db.collection(activeCollection).doc(auth.currentUser.uid).set({ alarms: nextAlarms }, { merge: true }).catch(() => {});
                        }
                    }
                    if (alarm.repeatDaily && alarm.enabled) {
                        ...
                        if (auth.currentUser) {
                            ...
                            if (activeCollection) {
                                db.collection(activeCollection).doc(auth.currentUser.uid).set({ alarms: disabledList }, { merge: true }).catch(() => {});
                            }
                        }
                    }
                }, delay);
                alarmTimersRef.current.set(alarm.id, [timeoutId]);
            }, [activeCollection, clearAlarmTimers]);
            ...
            useEffect(() => {
                if (!user?.uid || !quickTasks.length) return;
                const evaluateQuickTaskReminderFallbacks = () => {
                    ...
                    quickTasks.forEach(task => {
                        if (!task || !task.dueDate || task.completed) return;
                        const dueMs = new Date(`${task.dueDate}T${task.dueTime || '23:59'}`).getTime();
                    ...
                    Object.keys(nextMarks).forEach(key => {
                        ...
                        if (!activeKeys.has(key) || (typeof stamp === 'number' && (now - stamp) > (7 * 24 * 60 * 60 * 1000))) {
                ...
                const interval = setInterval(evaluateQuickTaskReminderFallbacks, 60000);
                return () => clearInterval(interval);
            }, [persistReminderMarks, quickTasks, user?.uid]);

            const effectiveUid = useMemo(() => masterUidOverride || (user ? user.uid : null), [user, masterUidOverride]);
            const discoveryCollections = useMemo(() => ([
                'tasks', 'users', 'userdata', 'notes',
                'faiora_data', 'user_metadata', 'metadata', 'profiles',
            ...
            const findRichestCollections = useCallback(async (uid, preferredCollections = []) => {
                const collections = Array.from(new Set([...(preferredCollections || []).filter(Boolean), ...discoveryCollections]));
                const results = await Promise.all(collections.map(coll => inspectCollectionForUid(coll, uid)));
                return results
                    .filter(result => result.exists && result.score > 0)
            ...
            const mergeCollectionPayloads = useCallback((summaries = []) => {
                ...
                (summaries || []).filter(Boolean).forEach((summary) => {
                    ...
                    if (!noteSections.length && Array.isArray(data.noteSections) && data.noteSections.length) {
                        noteSections = data.noteSections;
                    }
                    if (!alarms.length && data.alarms) {
                        alarms = Array.isArray(data.alarms) ? data.alarms : Object.values(data.alarms);
                    }
                    ...
                    if (!gamification && data.gamification && Object.keys(data.gamification).length) {
                ...
                return {
                    ...
                    profile,
                    settings,
                    gamification: gamification || { currentStreak: 0, longestStreak: 0, lastLoginDate: null, rewards: [] }
                };
            ...
            const consolidateCollections = useCallback(async (uid, summaries = [], preferredCollections = []) => {
                ...
                const targetCollections = Array.from(new Set([
            ...
            const hydrateResolvedPayload = useCallback(({
                ...
                const fetchedQuickTaskTrash = extractQuickTaskTrash(data)
                    .sort((a, b) => toComparableTime(b.deletedAt || b.updatedAt || b.createdAt) - toComparableTime(a.deletedAt || a.updatedAt || a.createdAt));
                const fetchedQuickTasks = extractQuickTasks(data)
                    .sort((a, b) => toComparableTime(b.createdAt || b.updatedAt) - toComparableTime(a.createdAt || a.updatedAt));
                ...
                const hasData =
                    (includeNotes && (fetchedNotes.length > 0 || fetchedTrash.length > 0 || fetchedSections.length > 0)) ||
                    (includeQuickTasks && (fetchedQuickTasks.length > 0 || fetchedQuickTaskTrash.length > 0 || fetchedAlarms.length > 0));
                if (markReady && hasData) {
                    coldBootNeedsUnifiedHydrationRef.current = false;
                    setIsFirstSyncDone(true);
                }

                console.log(`â˜ï¸ [SYNC] Hydrated payload from ${sourceCollection}. Tasks: ${includeQuickTasks ? fetchedQuickTasks.length : quickTasksRef.current.length}, Notes: ${includeNotes ? fetchedNotes.length : notesRef.current.length}`);
                return {
            ...
            // LABEL: POMODORO-STATE â€” Global persistence for the timer (v1.1.0)
            const [pomodoroTime, setPomodoroTime] = useState(25 * 60);
            const [isPomodoroActive, setIsPomodoroActive] = useState(false);
            const [pomodoroSessions, setPomodoroSessions] = useState(0);

            const isQuickTaskEditorOpen = isQuickTaskModalOpen || !!editingQuickTask;
            const isOverlayOpen = isCreatorOpen || isQuickTaskEditorOpen || isSetLockModalOpen || !!unlockingNote;
            ...
            /* handleRequestEditNote - opens note editor, handles locked notes */
            /* FIX 2026-04-15: Push history state for non-locked notes so back button */
            /* closes the note instead of navigating to the previous page (e.g. calendar) */
            ...
            const handleSetLock = async (hash, hint) => {
                if (!editingNote || !user) return;
                const updates = { isLocked: true, pinHash: hash, pinHint: hint };
                handleBulkUpdate(editingNote.id, updates);
                setEditingNote(prev => ({ ...prev, ...updates }));
                // Note will always require a PIN prompt when opened.
            ...
            // Pomodoro Logic (v1.1.4 Persistence & Functional Fixes) [FIX 2026-04-16]
            useEffect(() => {
            ...
            useEffect(() => {
                ...
                if (isPomodoroActive && pomodoroTime > 0) {
                    ...
                    // Push Notification for Pomodoro [FIX 2026-04-16]
            ...
            // FIX 2026-04-20: Alarm Watchdog â€” Ensures missed alarms (due to background throttling) trigger when app is touched/seen
            useEffect(() => {
                const alarmWatchdog = setInterval(() => {
                    ...
                    alarmsRef.current.forEach(alarm => {
                        ...
                        // If it's the exact time or within the last 1 minute, and hasn't been triggered in the last 2 minutes
                        if (alarm.time === nowTime) {
                            const lastTriggered = alarm.lastTriggeredAt || 0;
                            const diffMs = Date.now() - lastTriggered;
                            
                            // Check if this alarm is for today
            ...
            useEffect(() => {
                ...
                // Check on mount (delay a bit for SW)
                const timer = setTimeout(triggerCheck, 2000);
            ...
            // Ref to indicate if a modal was closed via code, to prevent double-handling by popstate
            ...
            useEffect(() => {
                 const handleBackButton = () => {
                     // If we're closing via code (close button), don't re-process
                     ...
                     // 3. Handle Set Lock modal
                     if (isSetLockModalOpen) {
                         setIsSetLockModalOpen(false);
                         return;
                     }

                     // 4. Handle Creator/Notepad Modal
                     if (isCreatorOpen) {
                         // If we are still in a 'creator' state after popping, it means we only closed a sub-popup
                     ...
                     // 5. Handle Quick Task Modal
                     if (isQuickTaskModalOpen || editingQuickTask) {
                         // Generic check for quicktask state
                     ...
                     /* FIX 2026-04-15: Advanced Back Button Handling
                        - Redirection to homepage (#/) if currently on a sub-page
                        - Scroll to top when navigating back to homepage
                        - Double-tap back button to exit app with toast confirmation
                     */
                     if (currentPath !== '/') {
                         // Not on home â€” navigate to home and scroll to top
                         redirectToHomeFromBack(false);
                         return;
                     }
                     if (now - prevPopState.current < 2000) {
                         prevPopState.current = 0;
                         exitApp();
                     } else {
            ...
            }, [editingQuickTask, exitApp, getHashPath, isCreatorOpen, isQuickTaskModalOpen, isSetLockModalOpen, redirectToHomeFromBack, showToast, unlockingNote]);
            ...
            // --- Utilities hoisted to App core ---

            // Centralized loading and syncing
            useEffect(() => {
                const uid = effectiveUid;
                if (!uid || !activeCollection) return;

                const hasWarmBootState =
                    notesRef.current.length > 0 ||
                    quickTasksRef.current.length > 0 ||
                    trashNotesRef.current.length > 0 ||
                    trashQuickTasksRef.current.length > 0;
                if (!hasWarmBootState) {
                ...
                const watchdogTimer = setTimeout(() => {
                    if (!isFirstSyncDone && uid && activeCollection && !watchdogTriggered) {
                        ...
                        db.collection(activeCollection).doc(uid).get({ source: 'server' }).then(snap => {
                            ...
                            console.error("âŒ [SYNC] Watchdog Force Fetch: Failed.", e.message);
                            setIsSyncHealthy(false);
                        });
                    }
                }, 2500);

                const unsubscribe = db.collection(activeCollection).doc(uid).onSnapshot(doc => {
                    ...
                    if (doc.exists) {
                        ...
                        const shouldDeferBlankColdBoot =
                            coldBootNeedsUnifiedHydrationRef.current &&
                            isProbingRef.current &&
                            !hasAnyRemoteContent;
                        const shouldDeferPartialBootSnapshot =
                            coldBootNeedsUnifiedHydrationRef.current &&
                            isProbingRef.current &&
                            roughTaskCount > 0 &&
                            roughNoteCount === 0;
                        ...
                        if (shouldDeferPartialBootSnapshot || shouldDeferBlankColdBoot) {
                            console.log(`â¸ï¸ [SYNC] Deferring cold-boot snapshot from ${activeCollection} until discovery resolves real data sources.`);
                            return;
                        }
                        ...
                        hydrateResolvedPayload({
                            ...
                            markReady: true,
                            includeQuickTasks: !quickTasksCollection || quickTasksCollection === activeCollection
                        });
                        ...
                        if (!isStreakCheckedRef.current) {
                            ...
                            if (cloudGamification.lastLoginDate !== today) {
                                ...
                                const milestones = { 
                                    3: '3-Day Spark', 7: '7-Day Flame', 14: '14-Day Blaze', 
                                    30: '30-Day Inferno', 60: '60-Day Supernova', 
                                    100: '100-Day Solar Flare', 365: '365-Day Eternal Flame' 
                                };
                    ...
                    // LABEL: SYNC-COMPLETE - Mark first sync as done AFTER data processing
                    ...
                    if (err.code === 'permission-denied') {
                        showToast("Sync: Permission Denied for UID " + uid);
                    } else {
                        if (showToast) showToast(`Sync Error: ${err.message}`);
                    }
                    // FIX 2026-04-17: Fallback to local data if cloud sync fails to prevent "stuck" preloader [Performance Optimization]
                    if (!coldBootNeedsUnifiedHydrationRef.current || !isProbingRef.current || notesRef.current.length > 0 || quickTasksRef.current.length > 0) {
                        setIsFirstSyncDone(true);
                    }
                });
                ...
                return () => unsubscribe();
            }, [activeCollection, effectiveUid, extractQuickTasks, getRichNoteMap, hydrateResolvedPayload, normalizeQuickTasks, quickTasksCollection, rescheduleAlarms]);

            useEffect(() => {
                const uid = effectiveUid;
                if (!uid || !quickTasksCollection || quickTasksCollection === activeCollection) return;
                ...
                return () => unsubscribe();
            }, [activeCollection, effectiveUid, hydrateResolvedPayload, quickTasksCollection]);
            ...
            // 5. Public Share Relay (Guest Sync to Owner's Private DB)
            useEffect(() => {
                if (!user || user.isAnonymous) return;

                const unsubscribe = db.collection('public_shares')
                    ...
                    .onSnapshot(snapshot => {
                        snapshot.docChanges().forEach(change => {
                            if (change.type === 'modified') {
                                ...
                                const coll = guestData.ownerCollection || activeCollection || 'tasks';

                                // We use a functional update to get current notes without adding it to dependency array
                                setNotes(currentNotes => {
                                    const localNote = currentNotes.find(n => n.id === noteId);
                                    if (localNote) {
                                        // Prevent loops by checking if incoming update matches local state
                                        ...
                                        if (isDifferent) {
                                            // Sync back to personal collection
            ...
            const handleUpdateNoteLocal = useCallback((updatedNote) => {
                setNotes(prev => {
                    ...
                    // Re-sort immediately: Pinned, sortOrder, updatedAt
                    newNotes.sort((a, b) => {
                        if (a.isPinned && !b.isPinned) return -1;
                        if (!a.isPinned && b.isPinned) return 1;
                        const sortA = a.sortOrder || 0;
            ...
            // LABEL: TRASH-CLEANUP - Periodically purges notes deleted > 30 days ago
            useEffect(() => {
                const cleanup = async () => {
                    if (!user || !activeCollection || trashNotes.length === 0) return;
                    ...
                    if (itemsToPurge.length > 0) {
                        ...
                        try {
                            await db.collection(activeCollection).doc(user.uid).update(updates);
                        } catch (e) {
                ...
                cleanup();
            }, [user, activeCollection, trashNotes]);
            ...
            }, [user, activeCollection, quickTasksCollection, trashQuickTasks]);

            // Bulk update: partially update a note by id (or create if isNew)
            const handleBulkUpdate = useCallback((noteId, updates, isNew = false) => {
                if (isProbing) {
                    console.warn('âš ï¸ [SYNC] Note update paused while discovering writable collection...');
                    return;
                }
                setNotes(prev => {
                    let newNotes;
                    if (isNew) {
                        newNotes = [...prev, { ...updates, updatedAt: new Date().toISOString() }];
                    } else {
                        newNotes = prev.map(n => n.id === noteId ? { ...n, ...updates, updatedAt: new Date().toISOString() } : n);
                    }
                ...
                if (user && activeCollection) {
                    if (isNew) {
                        ...
                        db.collection(activeCollection).doc(user.uid).update(fsUpdates).catch(e => console.warn('Bulk update sync fail', e));
                    }
                }
            }, [user, activeCollection, isProbing]);

            // Bulk delete: move note to Trash and sync to Firestore
            const handleBulkDelete = useCallback((noteId) => {
                if (isProbing) {
                    console.warn('âš ï¸ [SYNC] Note delete paused while discovering writable collection...');
                    return;
                }
                const noteToTrash = notes.find(n => n.id === noteId);
                handleDeleteNoteLocal(noteId);
                if (noteToTrash && user) {
                    const nextTrashNotes = [{ ...noteToTrash, deletedAt: new Date().toISOString() }, ...trashNotesRef.current.filter(note => note.id !== noteId)];
                ...
                if (user && activeCollection) {
                    ...
                    db.collection(activeCollection).doc(user.uid).update(deletePayload).catch(e => console.warn('Soft-delete sync fail', e));
                    showToast('Note moved to Trash');
                }
            }, [user, activeCollection, handleDeleteNoteLocal, isProbing, notes, showToast]);

            const handleRestoreNote = useCallback((noteId) => {
                const noteToRestore = trashNotes.find(n => n.id === noteId);
                if (!noteToRestore || !user || !activeCollection) return;
                
                // Cleanup deletedAt before restoring
            ...
            }, [user, activeCollection, trashNotes, showToast]);
            ...
            }, [user, activeCollection, showToast]);
            ...
            }, [user, activeCollection, showToast, isProbing]);
            ...
            const handleReorderNote = useCallback((noteId, targetNoteId) => {
                setNotes(prev => {
                    ...
                    // Update sortOrder for all notes in this new order
                    ...
                    // Sync to Firestore (Batch update sortOrder)
                    if (user && activeCollection) {
                        const updates = {};
                        updatedNotes.forEach(n => {
                            updates[`notes.${n.id}.sortOrder`] = n.sortOrder;
                        });
                        db.collection(activeCollection).doc(user.uid).update(updates).catch(e => console.warn('Sort sync fail', e));
                    }
                    return updatedNotes;
                });
            }, [user, activeCollection]);
            ...
            const handleReorderPriorityNote = useCallback((noteId, targetNoteId) => {
                setNotes(prev => {
                    ...
                    // Update homeOrder for these priority notes
                    ...
                    const newNotes = prev.map(n => {
                        if (priorityUpdates[n.id] !== undefined) {
                    ...
                    if (user && activeCollection) {
                        ...
                        db.collection(activeCollection).doc(user.uid).update(updates).catch(e => console.warn('HomeOrder sync fail', e));
                    }
                    return newNotes;
                });
            }, [user, activeCollection]);
                ...
                setNotes(prev => {
                    const newNotes = prev.map(n => {
                        if (n.id === noteId) {
                            ...
                            return { ...n, versions: [newVersion, ...versions].slice(0, 10) }; // Keep last 10
                        }
                        return n;
                    });
                    if (user) localStorage.setItem('faiora_notes_' + user.uid, JSON.stringify(newNotes));
            ...
            }, [user, activeCollection, notes, showToast]);
            ...
            const handleAddSection = useCallback((name) => {
                ...
                // Sync to Firestore
                if (user && activeCollection) {
                    db.collection(activeCollection).doc(user.uid).update({ noteSections: updated })
                        .catch(e => console.warn('Section add sync fail', e));
                }
            ...
            // --- Redundant Definitions Removed â€” Hoisted to top of App ---
            ...
            const handleUpdateQuickTasksLegacy = useCallback((updatedTasks, specificUpdate = null) => {
                ...
                if (currentUser) {
                    ...
                    if (taskCollection) {
                        ...
                        if (specificUpdate) {
                            // Atomic field update
                            docRef.update(specificUpdate)
                                .then(() => console.log('âœ… QuickTasks partial sync successful'))
                                .catch(e => {
                                    console.warn('âš ï¸ Partial sync failed, executing aggressive set(merge:true) fallback:', e.message);
                                    // Bypasses PERMISSION DENIED if it was caused by structural mismatch (Array vs Map)
                                    docRef.set({ quickTasks: updatedTasks }, { merge: true })
                                        .then(() => console.log('âœ… Aggressive fallback: QuickTasks sync successful'))
                                        .catch(err => {
                                            console.error('âŒ QuickTasks WRITE PERMISSION DENIED on', activeCollection, ':', err.message);
                            ...
                            docRef.set({ quickTasks: updatedTasks }, { merge: true })
                                .then(() => console.log('âœ… QuickTasks full sync successful'))
                                .catch(e => console.error('âŒ QuickTasks full sync FAILED:', e.message));
                        }
                    } else {
                        console.warn('âš ï¸ activeCollection is falsy - cannot sync quick tasks');
                    }
                } else {
                    console.warn('âš ï¸ No currentUser â€” cannot save quick tasks');
                }
            }, [activeCollection, isProbing, quickTasksCollection]);
            ...
            const handleAddAlarm = useCallback((payload) => {
                ...
                // FIX 2026-04-20: Support updating existing alarms with Samsung-style properties (days, snooze)
                if (payload.id) {
                    const updated = alarmsRef.current.map(a => a.id === payload.id ? { 
                        ...a, 
                        ...
                        enabled: true, // Fix 2026-04-22: Always enable alarm after editing
                        updatedAt: Date.now() 
                    } : a);
            ...
            const handleToggleAlarm = useCallback((alarmId) => {
                ...
                showToast(next?.enabled ? "Alarm enabled" : "Alarm disabled");
            }, [handleUpdateAlarms, showToast]);
            ...
            }, [dismissAlarmAlert, handleUpdateAlarms, showToast]);
            ...
            const handleUpdateQuickTask = (id, text, dueDate, dueTime) => {
                if (dueDate) {
                    FaioraNotifications.requestPermission().catch(() => {});
                }
                let dueTimestamp = null;
                ...
                const targetIndex = quickTasks.findIndex(t => t.id === id);
                if (targetIndex > -1) {
                    const updated = [...quickTasks];
                    const updatedTask = { ...updated[targetIndex], text, dueDate, dueTime, dueTimestamp, updatedAt: Date.now() };
            ...
            const handleDeleteQuickTask = (id) => {
                const targetIndex = quickTasks.findIndex(t => t.id === id);
                const taskToDelete = targetIndex > -1 ? quickTasks[targetIndex] : null;
            ...
            const handlePermanentDeleteQuickTask = useCallback((taskId) => {
                ...
                showToast('Quick task permanently deleted');
            }, [activeCollection, isProbing, quickTasksCollection, showToast]);
            ...
            const handleEmptyQuickTaskTrash = useCallback(() => {
                ...
                showToast('Quick-task trash emptied');
            }, [activeCollection, isProbing, quickTasksCollection, showToast]);
            ...
            const handleToggleQuickTask = (id) => {
                ...
                if (targetIndex > -1) {
                    ...
                    const updatedTask = {
                        ...
                        completedAt: !original.completed ? Date.now() : null
                    };
                    updated[targetIndex] = updatedTask;
                    handleUpdateQuickTasks(updated);
            ...
            // Fires as soon as Google popup/native sign-in resolves â€” before onAuthStateChanged in some browsers.
            useEffect(() => {
                const onSignedIn = (e) => {
                    ...
                    if (u) {
                        ...
                        const tryProbe = async () => {
                            ...
                            try {
                                ...
                                // 1. INSTANT LOAD: Set cached collection immediately so UI starts loading data
                                setActiveCollection(bestCollection);
                                setQuickTasksCollection(lastTaskColl);
                                
                                // FIX 2026-04-19: PREVENT PREMATURE READY SIGNAL [Sync Bug Fix]
                                // Only reveal app immediately if we actually have cached notes/tasks to show.
                                // If first sync isn't done and we have no local cache, keep skeletons visible.
                                ...
                                // 0. GLOBAL CONSENSUS: Check cloud metadata for master collection [NEW 2026-04-19]
                                try {
                                    ...
                                    console.warn("âš ï¸ [SYNC] Consensus check failed, falling back to local discovery race.");
                                }

                                const runProbe = async () => {
                                    // 2. FAST PATH: Check the cached collection first (Atomic check)
                                    try {
                                    ...
                                    try {
                                        ...
                                        if (candidates.length > 0) {
                                            ...
                                            const splitSources = Array.from(new Map(
                                                [richest, bestNoteSource, bestTaskSource]
                                                    .filter(Boolean)
                                                    .map(candidate => [candidate.coll, candidate])
                                            ).values());
                                            ...
                                            if (splitSources.length > 1 && (
                                                ...
                                                if (consolidated?.coll) {
                                                    ...
                                                    console.log(`âœ… [PROBE] Consolidated split data into '${consolidated.coll}'.`);
                                            ...
                                            // Handle read-only collections by migrating (if needed)
                                            try {
                                                ...
                                                for (const coll of discoveryCollections) {
                                                    ...
                                                    try {
                                                        ...
                                                        showToast("Data secured & permissions fixed!");
                                                        return coll;
                                                    } catch (e2) { /* next */ }
                                                }
                                                return richest.coll;
                                ...
                                // FIX 2026-04-17: Removed artificial delay for faster startup [Performance Optimization]
                                ...
                                coldBootNeedsUnifiedHydrationRef.current = false;
                                setIsProbing(false);
                                // FIX 2026-04-17: Probe Safety - Final check to ensure app is ready
                        ...
                        window.faiora_deep_scan = async () => {
                            ...
                            if (splitSources.length > 1 && (
                                ...
                                const consolidated = await consolidateCollections(u.uid, splitSources, [activeCollection, richest.coll]);
                                if (consolidated?.coll) {
                                    hydrateResolvedPayload({ uid: u.uid, data: consolidated.payload, sourceCollection: consolidated.coll, markReady: true });
                        ...
                        window.faiora_seed_samples = async () => {
                            ...
                            try {
                                await db.collection(activeCollection).doc(u.uid).set({
                                    notes: sampleNotes,
                                    quickTasks: sampleTasks
                                }, { merge: true });
                                console.log("âœ¨ [SUCCESS] Account seeded with sample data! Please refresh to see changes.");
                            } catch(e) {
                                console.error("âŒ [ERROR] Seeding failed:", e.message);
                            }
                        };
                        }, 0);
                    } else {
            ...
            useEffect(() => {
                ...
                // FIX 2026-04-17: FORCE READY FALLBACK - Ensure app is never stuck on skeletons for more than 8s [APK Performance Optimization]
            ...
            // FIX 2026-04-19: AUTOMATIC DEEP SEARCH [Brute Force Recovery]
            // If the app is ready but find 0 notes while tasks exist, automatically trigger a one-time
            // background deep scan to find data in legacy collections that the initial prober might have missed.
            useEffect(() => {
                // [NEW 2026-04-20] REDUCED SYNC AGGRESSION â€” Hyper-Turbo Step 3
                // The 'Deep Search' intensive sweep was over-triggering during boot, causing
                // network congestion and the '1-2 minute' loading stall.
                // We now wait until the primary discovery (isProbing) has fully settled.
                if (isProbing || !isFirstSyncDone || !user) return;
                
                const notesEmpty = notes.length === 0;
                const tasksExist = quickTasks.length > 0;
                const canRetry = Date.now() - lastDeepScan > 60000; // Only once per minute
            ...
            }, [isFirstSyncDone, isProbing, notes.length, quickTasks.length, user]);
            ...
            useEffect(() => {
                ...
                enforceSpeed();
                // Add listeners to reset if changed by context/extensions
                if (videoRef.current) {
            ...
            if (hash.startsWith('#/share/')) {
                ...
                return null; // Simplified from LoadingPage
            }

            // FIX 2026-04-15: Wait for auth check to finish before showing LoginModal
            // This prevents the Google login prompt from flashing when the user is already logged in.
            if (!isAuthChecked) {
                return null;
            }
            ...
            // ==========================================================================
            // LABEL: APP-RENDER â€” Main JSX Structure and Routing
            // ==========================================================================
            return (
                <HashRouter>
                    <FaioraErrorBoundary>
                    ...
                    <div className={isTransitioning ? 'page-transitioning' : ''}>
                        ...
                        <Route path="/stats" element={<StatsPage user={user} onOpenCreator={handleOpenCreator} quickTasks={quickTasks} pomodoroTime={pomodoroTime} isPomodoroActive={isPomodoroActive} />} />
                        <Route path="/streak" element={<StreakPage user={user} onOpenCreator={handleOpenCreator} gamification={gamification} pomodoroTime={pomodoroTime} isPomodoroActive={isPomodoroActive} />} />
                        <Route path="/alarms" element={<ClockPage user={user} onOpenCreator={handleOpenCreator} alarms={alarms} onAddAlarm={handleAddAlarm} onToggleAlarm={handleToggleAlarm} onDeleteAlarm={handleDeleteAlarm} pomodoroTime={pomodoroTime} setPomodoroTime={setPomodoroTime} isPomodoroActive={isPomodoroActive} setIsPomodoroActive={setIsPomodoroActive} pomodoroSessions={pomodoroSessions} alarmOverlayPermission={alarmOverlayPermission} onRequestAlarmOverlayPermission={requestAlarmOverlayPermission} onRefreshAlarmOverlayPermission={refreshAlarmOverlayPermission} hasNativeAlarmBridge={FaioraNotifications.hasNativeAlarmBridge ? FaioraNotifications.hasNativeAlarmBridge() : false} alarmsOnly={true} />} />
                        <Route path="/clock" element={<Navigate to="/alarms" replace />} />
                        <Route path="/profile" element={<EnhancedProfilePage user={user} onOpenCreator={handleOpenCreator} profileData={profileData} onSaveProfile={handleSaveProfile} quickTasks={quickTasks} gamification={gamification} pomodoroTime={pomodoroTime} setPomodoroTime={setPomodoroTime} isPomodoroActive={isPomodoroActive} setIsPomodoroActive={setIsPomodoroActive} pomodoroSessions={pomodoroSessions} activeCollection={activeCollection} cloudFieldsCount={cloudFieldsCount} showToast={showToast} isSyncHealthy={isSyncHealthy} />} />
                        <Route path="/settings" element={<SettingsPage user={user} onOpenCreator={handleOpenCreator} settingsData={settingsData} onSaveSettings={handleSaveSettings} pomodoroTime={pomodoroTime} isPomodoroActive={isPomodoroActive} showToast={showToast} />} />
                    ...
                    {editingQuickTask && (
                        <QuickTaskModal 
                            ...
                            onAdd={(text, date, time) => {
                                handleUpdateQuickTask(editingQuickTask.id, text, date, time);
                            }}
                        />
                    )}
                    ...
                    {/* LABEL: PERMISSION-MODAL-PORTAL â€” Prompt for overlay permissions */}
